...
- **Per-task check is the quick check, run once, at the end:** `rcmdcheck::rcmdcheck(build_args = "--no-build-vignettes", args = c("--as-cran", "--ignore-vignettes", "--no-manual"), env = c("_R_CHECK_CRAN_INCOMING_" = "false"))`. Its test phase runs the full testthat suite, so do not also run `devtools::test()` — use `testthat::test_file()` on a single file for fast iteration while developing. Compare the result against the standing reference set for this reduced surface; a task passes when no finding names a file it changed.
- **Full `--as-cran` with vignettes and the PDF manual happens only before a release**, nowhere else. For that run only, `pandoc` is not on `PATH` in a non-interactive `Rscript` session on this machine, so export `RSTUDIO_PANDOC=/usr/lib/rstudio/resources/app/bin/quarto/bin/tools/x86_64` (or put it on `PATH`) or `R CMD build` fails with "Pandoc is required to build R Markdown vignettes but not available".
