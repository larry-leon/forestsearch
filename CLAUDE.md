...
- **Verification per task is the task's own acceptance tests, and nothing else.** Write them into `tests/testthat/`, run them with `devtools::load_all()` + `testthat::test_file()`, and gate on those files being green. **Do not run the full `devtools::test()` suite, and do not run `R CMD check` or `rcmdcheck` in any form, as part of a task.**
- **The full suite and any CRAN check run only when Larry explicitly asks** — typically before an install or a release, never otherwise. Two check surfaces exist for that case. The certification surface is bare `R CMD check --as-cran` (via `rcmdcheck::rcmdcheck(args = "--as-cran")`) — it builds the PDF manual, so it catches LaTeX-unsafe Rd content that other surfaces miss. `devtools::check()` is *not* certification: its `manual = FALSE` default passes `--no-manual`, skipping the PDF manual build, so it can report `0 errors | 0 warnings | 0 notes` on a tree the certification surface flags. For either run, `pandoc` is not on `PATH` in a non-interactive `Rscript` session on this machine, so export `RSTUDIO_PANDOC=/usr/lib/rstudio/resources/app/bin/quarto/bin/tools/x86_64` (or put it on `PATH`) or `R CMD build` fails with "Pandoc is required to build R Markdown vignettes but not available".
