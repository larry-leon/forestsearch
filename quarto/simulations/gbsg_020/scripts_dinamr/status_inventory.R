...
rules <- list(
  ...
  c("results/*_s5rerun_*.rds", "`s5rerun`: the 18 Section 5 FS cells re-run under the aligned package, 2,000 replicates each (batch and combined)"),
  c("results/*_s5rerunchk_*.rds", "`s5rerun` Step 2 check: A7, 200 replicates"),
  c("results/*_s5smk1500_*.rds", "`s5rerun` n 1500 memory smoke: 31% HR 1.75 n 1500, 13 replicates"),
  ...
  c("s5rerun/**", "`s5rerun` driver, runner, recorder, paired read-out, ranges, logs (`WALL_SECONDS`, `CELL DONE wall=`, worker build checks, memory samples)"),
  ...
  c("s5rerun*_*.html", "`s5rerun` batch and combine renders, and the Step 2 check render"),
  c("s5smk1500_*.html", "`s5rerun` n 1500 memory-smoke render"),
