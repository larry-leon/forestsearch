...
camps <- list(
  ...
  # nullmr (TASK_null_gbsg_mr_2026-09-21): the structural null at the nullid
  # screen with MR on.  Batch-only.  Its smoke (`nullmrsmoke`) bundles are
  # quickrun-tagged and outside bre; `^nullmr_` does not match their renders.
  nullmr = list(bdir = "results", bre = "^(fs|dina|grf)_.*_null[0-9]{3}_nb20_nullmr_(res|combined)_[0-9]+_[0-9]+[.]rds$",
                rdir = ".", rre = "^nullmr_.*[.]html$",
                scripts = "scripts_dinamr", sre = "nullmr",
                docre = "^(REPORT|summary)_null_gbsg_mr[_.]"),
  # declcal (TASK_declcal_CAMPAIGN_2026-09-22_v2): the declaration-calibration
  # finite-sample evaluation, FS only, blocks bnull / inull / power.  Batch-only;
  # its payloads are the section-5 schema, not the template recorder, so
  # .norm_bundle() below maps their meta names.  The pilot bundle
  # (declcal_pilot_*_res_1_200) is outside bre.  Its report lives in dev/reports/.
  declcal = list(bdir = "results", bre = "^declcal_(bnull|inull|power)_[A-C][0-9]_res_1_2000[.]rds$",
                rdir = ".", rre = "^declcal_.*[.]html$",
                scripts = "scripts_dinamr", sre = "declcal([^c]|$)",
                docre = "^(REPORT|summary)_declaration_calibration[_.]"),
  # declcalc0 (TASK_declcal_c0_campaign_2026-09-22): the declcal B and C blocks
  # re-run with declaration_c0 = c(0.70, 0.75, 0.80, 0.85), same seeds; payloads
  # in the declcal schema plus per-c0 columns.  The smoke (declcalc0_smoke_*) is
  # outside bre.  Its report lives in dev/reports/.
  declcalc0 = list(bdir = "results", bre = "^declcalc0_(inull|power)_[BC][0-9]_res_1_2000[.]rds$",
                rdir = ".", rre = "^declcalc0_.*[.]html$",
                scripts = "scripts_dinamr", sre = "declcalc0",
                docre = "^(REPORT|summary)_declcal_c0[_.]"),
  # mrs5 (TASK_mr_alignment_section5_probe / _sweep_2026-09-23): the MR admission
  # alignment's after arm on the 18 Section 5 cells, FS only, 200 replicates per
  # cell, batch-only.  Tags mrs5probe (A7) and mrs5sweep (the other 17).  The
  # Gate T (res_1_10) and parent-build check (mrs5pre) bundles are outside bre.
  # Its reports live in dev/reports/.
  mrs5 = list(bdir = "results", bre = "^fs_.*_nb20_mrs5(probe|sweep)_res_1_200[.]rds$",
                rdir = ".", rre = "^mrs5(probe|sweep|pre)_.*[.]html$",
                scripts = "mrs5sweep", sre = ".",
                docre = "^(REPORT|summary)_mr_alignment_section5[_.]"),
  # s5rerun (TASK_section5_full_rerun_2026-09-24): the 18 Section 5 FS cells
  # re-run in full (2,000 replicates, batch 1 / batch 1001 / combine) under the
  # aligned package, beside the committed p12x20 / cert20 / e1stud bundles.  The
  # Step 2 check (s5rerunchk, A7 200) and the n1500 memory smoke (s5smk1500) are
  # outside bre.  Its report lives in dev/reports/.
  s5rerun = list(bdir = "results", bre = "^fs_.*_nb20_s5rerun_(res|combined)_[0-9]+_[0-9]+[.]rds$",
                rdir = ".", rre = "^(s5rerun|s5rerunchk|s5smk1500)_.*[.]html$",
                scripts = "s5rerun", sre = ".",
                docre = "^(REPORT|summary)_section5_full_rerun[_.]"))

# declcal payloads name n / target_hr / rep where the template bundles say
# n_sample / target_hr_harm / sim_id, and carry no subgroup_method or
# mr_inference (FS only; MR post-selection correction off, field capture only).
# Identity on every template bundle.
.norm_bundle <- function(b) {
  m <- b$meta
  m$n_sample <- m$n_sample %||% m$n
  m$target_hr_harm <- m$target_hr_harm %||% m$target_hr
  if (grepl("declcal", m$task %||% "")) {
    m$subgroup_method <- m$subgroup_method %||% "consistency"
    m$mr_inference <- m$mr_inference %||% FALSE
  }
  if (is.null(b$results$sim_id) && !is.null(b$results$rep)) b$results$sim_id <- b$results$rep
  b$meta <- m
  b
}
...
for (cn in names(camps)) {
  ...
  bms <- if (batch_only) lapply(bat, function(f) .norm_bundle(readRDS(file.path(cc$bdir, f)))) else list()
  eng <- if (batch_only) {
    bm_ <- lapply(bms, `[[`, "meta")
  ...
  if (batch_only) {
    ...
    for (kk in unique(key)) {
      ...
      meta_rows[[length(meta_rows) + 1]] <- data.frame(
        campaign = cn, cell = m$cell_id %||% "—", prev = m$harm_prevalence_super, hr = m$target_hr_harm, n = m$n_sample,
...
for (cn in names(camps)) {
  specs <- switch(cn,
    ...
    nullmr   = c(file.path(rq, "results/*_nb20_nullmr_*"), file.path(rq, "nullmr_*.html")),
    declcal  = c(file.path(rq, "results/declcal_*")),
    declcalc0 = c(file.path(rq, "results/declcalc0_*")),
    mrs5      = c(file.path(rq, "results/*_mrs5*"), file.path(rq, "mrs5*")),
    s5rerun   = c(file.path(rq, "results/*_s5rerun_*"), file.path(rq, "results/*_s5rerunchk_*"),
                  file.path(rq, "results/*_s5smk1500_*"), file.path(rq, "s5rerun*"), file.path(rq, "s5smk1500_*")))
