# declcal_c0approx_run.R -- one cell of the 60-replicate c0 capture for the
# approximate c0 table (TASK_declcal_c0_approx_2026-09-22).  Run per B cell
# with FS_S7_NSIMS=20 (reps 1-20), DECLCAL_BCAL=2000, the declcal.sh per-cell
# environment otherwise; post-processed by declcal_c0approx_findings.R.
#
# TRANSPLANT of the committed declcal_run.R (at 80abee8d).  Lines changed
# (declcal_run.R line numbers), and nothing else:
#   (a1) :57  n_sims default 2000L -> 20L (the replicate range; 1-20).
#   (a2) :73  campaign tag default "declcal" -> "declcal_c0approx".
#   (a3) :326 fs_mr_inference(..., declaration_c0 = c0_grid) -- the argument.
#   (a4) after :213 the c0 grid and the per-c0 record columns appended to the
#        schema; after :354 those columns filled from fld$Mstar_c0 (the same
#        q() quantile rule as the unshifted kappa_hat).
#   (a5) :419 meta$task names this task and records the c0 grid.
# B_cal is set by DECLCAL_BCAL=2000 (declcal_run.R's own default, unchanged).
...
n_sims       <- .env_int("FS_S7_NSIMS", 20L)                               # (a1)
...
campaign_tag <- .env_chr("FS_S7_CAMPAIGN", "declcal_c0approx")            # (a2)
...
# (a4) the c0 grid (natural HR scale) and the per-c0 record columns
...
c0_cols <- c("kappa_hat_05", "kappa_hat_10", "pstar_implied_05",
             "Mstar_c0_q90", "Mstar_c0_q95", "Mstar_c0_q99", "fw_1645")
...
record_replicate <- function(sim_id) {
  ...
  for (sel_i in order(lengths(fam), decreasing = TRUE)[1:3]) {
    ...
    mr <- ns$fs_mr_inference(
      ...
      keep_field_matrix = FALSE, declaration_c0 = c0_grid)               # (a3)
  ...
  # ---- (a4) per-c0 columns from the shifted maxima Mstar_c0 ----------------
  Mc <- fld$Mstar_c0
  ...
  for (j in seq_along(c0_grid)) {
    ...
    r[[paste0("kappa_hat_10_", sx)]] <- q(0.90, Mj)
...
.save <- function(final) {
  ...
  payload <- list(
    ...
    meta = list(task = "TASK_declcal_c0_approx_2026-09-22", cell_id = cell_id,   # (a5)
