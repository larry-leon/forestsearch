# declcalc0_run.R -- one cell of the declcal re-run with the clinically
# specified null level c0 (TASK_declcal_c0_campaign_2026-09-22).  Called by
# declcalc0.sh.
#
# TRANSPLANT of the committed declcal_run.R (at 1f7dc5b3).  Named changes, and
# nothing else (task section 7):
#   (c0-1) declaration_c0 = c(0.70, 0.75, 0.80, 0.85) passed to the field
#          capture (fs_mr_inference); the capture returns Mstar_c0 (B x 4).
#   (c0-2) the per-c0 columns appended to the section-5 schema, suffixed
#          c070 / c075 / c080 / c085 (task section 3), and two aux flags for
#          the section-4 monotonicity assertions.
#   (c0-3) the identity gate of task section 4 against the committed
#          declcal payload (exit 6), after the unchanged fidelity gate.
#   (c0-4) campaign tag default "declcalc0"; meta$task names this task.
# The declcal_run.R header follows, unchanged.
#
...
campaign_tag <- .env_chr("FS_S7_CAMPAIGN", "declcalc0")                  # (c0-4)
...
record_replicate <- function(sim_id) {
  ...
  aux <- list(rep = sim_id, search_declared = NA_integer_, msg = NA_character_,
              ...
              c0_mono_draws = NA, c0_kappa_le = NA)                     # (c0-2)
  t0 <- proc.time()[3]
  ...
  for (sel_i in order(lengths(fam), decreasing = TRUE)[1:3]) {
    ...
    mr <- ns$fs_mr_inference(
      ...
      keep_field_matrix = FALSE, declaration_c0 = c0_grid)               # (c0-1)
  ...
  # ---- (c0-2) per-c0 columns from the shifted maxima Mstar_c0 --------------
  Mc <- fld$Mstar_c0
  if (is.null(Mc) || !identical(dim(Mc), c(length(Ms), length(c0_grid))) ||
      !identical(colnames(Mc), as.character(c0_grid)))
    stop("Mstar_c0 not captured as B x 4 with the c0 grid's column names")
  for (j in seq_along(c0_grid)) {
    Mj <- Mc[, j]; sx <- c0_sfx[j]
    ...
    r[[paste0("Mstar_c0_q99_", sx)]] <- q(0.99, Mj)
  }
  # task section 4: per draw, Mstar_c0 non-increasing as c0 decreases (the
  # unshifted Mstar is the c0 = c2 end); kappa_hat_05_c0 <= kappa_hat_05.
  Mfull <- cbind(Mc[, order(c0_grid), drop = FALSE], Ms)
  aux$c0_mono_draws <- all(Mfull[, -ncol(Mfull), drop = FALSE] <= Mfull[, -1L, drop = FALSE])
  aux$c0_kappa_le <- all(vapply(c0_sfx, function(sx) r[[paste0("kappa_hat_05_", sx)]], 0) <= r$kappa_hat_05)
...
.save <- function(final) {
  ...
  payload <- list(
    ...
    meta = list(task = "TASK_declcal_c0_campaign_2026-09-22", cell_id = cell_id,   # (c0-4)
                declaration_c0 = c0_grid, c0_suffix = c0_sfx,
...
# ---- (c0-3) identity gate (task section 4) ------------------------------------
# Against the committed declcal payload of the same cell (at 81752681), matched
# by rep.  Any disagreement, or a failed monotonicity assertion, is a STOP.
ref_blk  <- c(B = "inull", C = "power")[[substr(cell_id, 1L, 1L)]]
...
id_bad <- list()
if (any(is.na(mref$rep)) || any(!ok) || any(mref$status != "ok"))
  id_bad[["status/rep"]] <- rows$rep[is.na(mref$rep) | !ok | mref$status != "ok"]
for (v in id_cols) {
  a <- rows[[v]]; b <- mref[[v]]
  same <- (is.na(a) & is.na(b)) | (!is.na(a) & !is.na(b) & a == b)
  if (any(!same)) id_bad[[v]] <- rows$rep[!same]
}
am <- match(rows$rep, aux$rep)
if (any(!(aux$c0_mono_draws[am] %in% TRUE))) id_bad[["c0_mono_draws"]] <- rows$rep[!(aux$c0_mono_draws[am] %in% TRUE)]
if (any(!(aux$c0_kappa_le[am] %in% TRUE)))   id_bad[["c0_kappa_le"]]   <- rows$rep[!(aux$c0_kappa_le[am] %in% TRUE)]
cat(sprintf("IDENTITY: %d reps vs %s on %d columns + 2 monotonicity checks: %s\n",
            nrow(rows), ref_path, length(id_cols),
            if (!length(id_bad)) "all identical, monotonicity holds" else
              paste(sprintf("%s disagree on reps %s", names(id_bad),
                            vapply(id_bad, function(x) paste(x, collapse = ","), "")), collapse = " ; ")))
c0_rate <- function(p) paste(sprintf("%s %.4f", c0_sfx, vapply(c0_sfx, function(s) mean(rows[[paste0(p, "_", s)]][ok]), 0)), collapse = " ")
cat(sprintf("RATES_C0 cal05: %s\n", c0_rate("declared_cal05")))
cat(sprintf("RATES_C0 cal10: %s\n", c0_rate("declared_cal10")))
if (length(id_bad)) quit(status = 6L)
