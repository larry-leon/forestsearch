#!/bin/bash
# declcalc0 driver (TASK_declcal_c0_campaign_2026-09-22): the declcal B and C
# blocks re-run with declaration_c0 = c(0.70, 0.75, 0.80, 0.85).  TRANSPLANT of
# the committed declcal.sh (at 1f7dc5b3).  Named changes, and nothing else:
#   (c0-a) the per-cell runner is declcalc0_run.R; halt records name this task.
#   (c0-b) exit 6 from declcalc0_run.R (identity gate, task section 4) STOPS
#          THE CAMPAIGN with the disagreeing replicates recorded, as exit 3 does.
#   (c0-c) the per-cell summary grep also shows the IDENTITY and RATES_C0 lines;
#          the block commit message names the declcalc0 campaign.
# The declcal.sh header follows, unchanged.
#
...
if (( ! DRY )); then
  ...
  ( sleep $TIMEOUT_S
    ...
    pkill -f "declcalc0_run.R"
    kill $MAINPID ) &
  WATCHDOG=$!
fi
stop_run () { [[ -n "$WATCHDOG" ]] && kill $WATCHDOG 2>/dev/null; exit $1; }
echo "=== DECLCALC0 START $(date) ; mode $MODE ; B_cal $BCAL ; reps $REPS ; per-rep cap ${CAP_S}s ; campaign hard cap ${TIMEOUT_S}s ; watchdog pid ${WATCHDOG:-none} ; dry=$DRY ; blocks: ${FILES[*]} ==="
...
halt_record () {  # $1=camp $2=cell $3=stage $4=detail $5=action ; echoes the file name
  local F="$QD/HALT_$1_$2_$(date +%Y-%m-%d).txt"
  {
    ...
    echo "  task     : TASK_declcal_c0_campaign_2026-09-22"
    echo "  when     : $(date)"
...
for CF in "${FILES[@]}"; do
  ...
  while read -r CELL DGM HR N LAB; do
    ...
    free -g | awk '/^Mem:|^Swap:/'
    CT0=$(date +%s)
    ( cd "$QD" && env "${UNSET[@]}" "${KN[@]}" Rscript "$SP/declcalc0_run.R" ) > "$LOG" 2>&1 < /dev/null
    RC=$?
    echo "CELL END: $CAMP $CELL rc=$RC wall=$(( $(date +%s) - CT0 ))s"
    grep -E '^(CELL|FIDELITY|IDENTITY|RATES|chunk)' "$LOG"
    ...
    if (( RC == 3 )); then
      ...
      HF=$(halt_record $CAMP $CELL "IDENTITY GATE" "c0-independent columns differ from the committed declcal payload, or a c0 monotonicity assertion failed; replicates on the IDENTITY line of $LOG" "CAMPAIGN STOPPED (task section 4): the new columns are interpretable only if this gate holds.")
      commit_paths "$CAMP $CELL: IDENTITY GATE FAILED -- halt record, payload and log" "$RQ/$(basename "$HF")" "${BPATHS[@]}"
      echo "=== IDENTITY GATE FAILED at $CAMP $CELL; campaign stopped ==="
      stop_run 6
    elif (( RC == 4 )); then
      ...
      HF=$(halt_record $CAMP $CELL "run (rc $RC)" "declcalc0_run.R failed; see $LOG" "this cell is abandoned; the campaign continues.")
      BPATHS+=("$RQ/$(basename "$HF")"); FAILED+=("$CAMP:$CELL"); continue
    fi
    DONE+=("$CAMP:$CELL")
  done < "$CF"
  (( DRY )) || commit_paths "data(sims): add declcalc0 campaign payloads -- block $CAMP" "${BPATHS[@]}"
done
echo "DECLCALC0 COMPLETE $(date) total_wall=$(( $(date +%s) - T0 ))s ; completed ${#DONE[@]} cells: ${DONE[*]:-none} ; failed ${#FAILED[@]}: ${FAILED[*]:-none}"
stop_run 0
