...
Date: 2026-09-08. Task: `dev/tasks/TASK_complement_location_2026-09-08.md` (committed as received; the `_v2` file named in the invocation was not present in `~/Downloads`, so the non-v2 file is the committed spec and the Stage 0 post-merge gate below is taken from the invocation text). Executor: Claude Code (Linux). **Report-and-wait: no repair proposal, no recommendation.** No compute beyond the 5-replicate identity gate; no `R/` change; no campaign bundle written.
...
1. The `_v2` task file named in the invocation was not present in `~/Downloads`; the non-v2 file (the only copy) is committed as received and archived, and Stage 0 follows the invocation text. If a v2 exists elsewhere, its deltas beyond Stage 0 are not reflected here.
