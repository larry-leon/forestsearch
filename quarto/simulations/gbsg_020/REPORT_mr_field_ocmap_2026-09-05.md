...
1. **The field's one-sided bound does NOT hold across the whole map** *(corrected per Larry's review, 2026-09-05)*: c3 (h1.00, n=1000) sits at **0.920** against β(Ĥ) — four Wilson-SEs below nominal, against 0.963 in the same regime at n=500. This is the screen-conditioning effect: the plug-in field is more detectable than the truth, and the field's conservatism, which masks the effect at n=500 (λ-SD/SD 1.41), has thinned to 1.23 at n=1000 and no longer covers it. The prediction on the record: further degradation of the tie-regime one-sided bound at larger n. The other four cells stand as reported: 0.965 (c1), 0.953 (c2), 0.981 (c4), 0.956 (c5).
2. **Moderate harm at large n (c5) is the first regime where MR (IJ)'s own two-sided interval under-covers**: 0.901 (0.887, 0.913) with SE/SD 1.08 — the conservatism that propped up IJ's two-sided coverage elsewhere (SE/SD 1.3–2.0) is gone at n=1500, and the field's plain two-sided interval is lower still (0.848). At n=500 (c1) IJ still covers (0.977) but only via SE/SD 1.43. Note that IJ's SE/SD of 1.08 here is not the separated-regime limit (≈2): re-selection mass is spread over overlapping candidates, so the IJ variance sits near the plain sampling variance rather than the winner's-curse-inflated one.
3. **The protective cell (c2) is a naive catastrophe and an MR success**: naive conditional coverage 0.001 with retained bias +0.91 log-HR (+6.1 SD); MR (IJ) 0.984 and MR (field) 0.988 two-sided, 0.964/0.953 one-sided. The sweep-era concern about IJ's conditional coverage under a protective truth is **not tested here** *(corrected per Larry's review, 2026-09-05)*: that statement concerned the detected set shrinking as n grows toward 2,000, whereas c2 is n=500 at 52% detection.
