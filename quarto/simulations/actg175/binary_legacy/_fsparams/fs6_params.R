...
# fs6_params.R
#
# Canonical FS parameter bundle "fs6" for ACTG175 binary harm simulations.
#
# Sourced by per-config qmds at:
#   quarto/simulations/actg175/actg175_binary_m1_harm_<focus>_fs6.qmd
#
# Provides one function: get_fs6_params(sg_focus, ...)
...
#
# -----------------------------------------------------------------------------
# Regime: fs6 — permissive search, broad candidate space, full cut retention.
#
...
#
# Key choices:
#   max_subgroups_search = 10  → broad candidate space (vs. fs6's 5)
...
#                            applied inside get_fs6_params())
#
# To create a sibling bundle "fs6" with different parameter choices:
#   1. Copy this file to fs6_params.R
#   2. Rename the function to get_fs6_params()
#   3. Edit the values that differ
#   4. Update the regime description above
#   5. Use clone_config.sh fs6 fs6 to spawn the parallel sg_focus qmds
# =============================================================================

#' Get FS parameters for the "fs6" bundle
#'
#' Returns the FS parameter list used by ACTG175 binary harm simulations
#' under the "fs6" regime. The bundle's \code{stop_threshold} is set to
...
get_fs6_params <- function(sg_focus,
                           outcome_name = "y_sim",
  ...
  list(
    ...
    max_subgroups_search      = 50,        # fs6: broad
    use_twostage              = TRUE,
    ...
    # stop_threshold: 0.90 is the fs6 bundle's value. Package requires
    # NULL when sg_focus uses neighborhood-based selection (hrMaxSG /
    # hrMinSG); applied here so qmds can call get_fs6_params() and use
    ...
    conf.cont_jcuts        = list(cd40 = 5, wtkg = 5, preanti = 5),
