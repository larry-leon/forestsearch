...
#   ORSG_WORKERS=<W> ORSG_TIMEOUT=<s> ORSG_CEILING=<s> setsid nohup bash <dir>/scripts_or/run_or.sh \
#     > <dir>/logs_or/runner.log 2>&1 &
...
CAMPAIGNS=("orfs consistency fs" "orgrf grf grf" "ordina dina dina")
# The six cells, in the task's order: <design-token> <FS_OR_TARGET> <n>
CELLS=("or075 0.75 500" "or075 0.75 2000" "or150 1.5 500" "or150 1.5 2000" "or100 1.0 500" "or100 1.0 2000")
...
log "PREFLIGHT OK: branch $BRANCH, HEAD $(git rev-parse --short HEAD), workers $ORSG_WORKERS, timeout ${ORSG_TIMEOUT}s, ceiling ${ORSG_CEILING}s, Built '$BUILT'"
heartbeat campaign start "HEAD=$(git rev-parse --short HEAD) workers=$ORSG_WORKERS timeout_s=$ORSG_TIMEOUT ceiling_s=$ORSG_CEILING built=$BUILT"
...
for cspec in "${CAMPAIGNS[@]}"; do
  ...
  for spec in "${CELLS[@]}"; do
    ...
    if tracked_clean "$RQ/mr_or_harm/${STEM}_d5000/${STEM}_combined_1_2000.rds"; then
    ...
    rm -rf "$RDIR"
    rm -f "$QMD/${STEM}_batch_1_1000.html" "$QMD/${STEM}_batch_1001_2000.html" "$QMD/${STEM}_combine_1_2000.html"
    log "START $CELL: identifier $METH target_or_h $TARGET n $N ($STEM)"
    render "$CELL" "$STEM" batch_1_1000    batch   1    1000 "$METH" "$TARGET" "$N" "$CAMP"
    render "$CELL" "$STEM" batch_1001_2000 batch   1001 1000 "$METH" "$TARGET" "$N" "$CAMP"
    render "$CELL" "$STEM" combine_1_2000  combine 1    1000 "$METH" "$TARGET" "$N" "$CAMP"
    mv "$QMD/${STEM}_batch_1_1000.html" "$QMD/${STEM}_batch_1001_2000.html" "$LOGD/" 2>/dev/null
    # Gate 2 (Stage 2's per-cell gate)
    (cd "$HERE" && ORSG_DIR=.. ORSG_WORKERS=$ORSG_WORKERS Rscript gate2.R "$TARGET" "$N" "$CELL" "$CAMP") > "$GREC" 2>&1
    GRC=$?
    ...
    [ $GRC -eq 0 ] || halt "$CELL" "gate2.R failed: ${COUNTS:-no GATE_COUNTS line}" "$GREC"
    {
      ...
      echo "- Seeds: the study's pre-generated table indexed by global sim_id (seed_base 8316951); batches sim_id 1-1000 and 1001-2000, then combine."
    ...
    PATHS=("$RQ/mr_or_harm/${STEM}_d5000" "$RQ/${STEM}_combine_1_2000.html" "$RQ/REPORT_actg175_or_gate2_${DATE}.md" "$RQ/LOG_${TAG}_progress.txt")
    heartbeat "$CELL" done "cell_wall_s=$(( $(date +%s) - CELL_T0 )) $COUNTS"
    commit "actg175 or $CELL: $METH, target_or_h $TARGET, n $N, 2000 replicates + combine; Gate 2 PASS ($COUNTS)" "${PATHS[@]}" || { log "CELL COMMIT FAILED: $CELL"; exit 1; }
...
commit "actg175 or: campaigns orfs, orgrf, ordina complete (cumulative render wall ${CUM} s)" "$RQ/LOG_${TAG}_progress.txt" || log "final heartbeat commit failed"
