...
record_replicate <- function(sim_id) {
  ...
  # Exact beta(Hhat)/beta(Hhat^c) via the package eval API; naive + MR columns
  # from the fit; classification on the analysis sample.
  att <- fs_attach_betaHhat(fs.est, eval_df = eval_df,
                            outcome_type = "continuous")
  rec$betaHhat_H  <- att$betaHhat_H;  rec$betaHhat_Hc <- att$betaHhat_Hc
  rec$sg_def <- att$sg_def %||% fs.est$sg.harm$definition %||% NA_character_
  rec$n_sel  <- att$nH_eval %||% NA_integer_
  sgv <- fs.est$sg.harm$flag %||% fs.est$df_flag$treat.recommend %||% NULL
  if (!is.null(sgv)) {
    rec$n_harm <- sum(sgv == 1L, na.rm = TRUE)
    rec[c("sens","spec","ppv","npv")] <- as.list(.classify(sgv, df[[harm_col]]))
  }
  for (nm in c("nv_H_est","nv_H_lo","nv_H_hi","nv_H_se",
               "mr_H_est","mr_H_lo","mr_H_hi","mr_H_se_ij",
               "mr_Hc_est","mr_Hc_lo","mr_Hc_hi","mr_Hc_se_ij","ij_source"))
    rec[[nm]] <- fs.est[[nm]] %||% fs.est$mr[[nm]] %||% rec[[nm]]
  rec$mr_ok <- as.integer(!is.na(rec$mr_H_est))
  ...
  if (run_fb) {
    ...
    if (!is.null(fb))
      for (nm in c("fb_H_est","fb_H_lo","fb_H_hi","fb_H_se"))
        rec[[nm]] <- fb[[nm]] %||% fb$H2[[nm]] %||% rec[[nm]]
  }
  rec
}
```
