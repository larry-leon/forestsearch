...
if (nrow(rd)) {
  rows <- lapply(names(est_keys), function(lab) {
    k   <- est_keys[[lab]]
    e   <- rd[[paste0(k, "_H_est")]]
    lo  <- rd[[paste0(k, "_H_lo")]]
    hi  <- rd[[paste0(k, "_H_hi")]]
    se  <- rd[[.se_col(k)]] %||% rep(NA_real_, nrow(rd))
    ref <- .ref_for(k)
    data.frame(Estimator = lab,
               n        = sum(is.finite(e)),
               Avg      = .mfin(e),
               Bias     = .mfin(e - ref),
               SD_emp   = .sdfin(e),
               SE_hat   = .mfin(se),
               Coverage = .cover(ref, lo, hi),
               Width    = .mfin(hi - lo),
               stringsAsFactors = FALSE)
  })
  est_tab <- do.call(rbind, rows)
  gt(est_tab) |>
    tab_header(
      title = if (run_fb) "MR vs FB bootstrap: estimation & coverage"
              else "MR: estimation & coverage",
      subtitle = sprintf(
        "MD/harm cell, quick run - n=%d, %d sims, B=%s, draws=%d - resample identification",
        n_sample, n_batch, if (run_fb) as.character(nb_boots) else "0 (skipped)",
        mr_draws)) |>
    fmt_number(columns = c(Avg, Bias, SD_emp, SE_hat, Width), decimals = 2) |>
    fmt_number(columns = Coverage, decimals = 2) |>
    cols_label(n = "n", Avg = "Mean est.", Bias = "Bias vs referent",
               SD_emp = "SD (emp)", SE_hat = "Mean SE (model)",
               Coverage = "95% cov.", Width = "Mean CI width") |>
    tab_footnote(footnote = md(paste(
      "All columns oriented (positive = harm).",
      "Referents: **oracle** vs the oriented DGM effect in the TRUE region",
      sprintf("($+%.0f$, the oriented `effect_Q`; the oracle refits on $Q$, so",
              abs(truth$effect_Q)),
      "$\\beta(\\widehat H)$ is not its target);",
      "**naive/FB/MR** vs the oriented per-replicate exact $\\beta(\\widehat H)$",
      "computed on the DGM super-population (zero Monte Carlo error)."))) |>
    tab_footnote(footnote = md(paste(
      "**FB row caveat (unconditional comparator).**  The candidate family",
      "REGENERATES under bootstrap resampling even for the consistency engine",
      "(quantile-derived cuts move with each resample;",
      "`dev/glm-continuous-sims/V_fb_adjudication.md` section V3, corroborated by",
      "`.fs_family_status()`).  FB therefore targets the *unconditional* estimand",
      "while MR conditions on the fitted family, so the FB row's",
      "$\\beta(\\widehat H)$ comparison is reported with that caveat -- an FB-MR",
      "gap is a finding about the estimands, not an error in either."))) |>
...
# Median companion, same detected replicates, location columns are medians;
# SD, n, and coverage computed exactly as above (mirrors the combine document).
if (nrow(rd)) {
  rows_med <- lapply(names(est_keys), function(lab) {
    k   <- est_keys[[lab]]
    e   <- rd[[paste0(k, "_H_est")]]
    lo  <- rd[[paste0(k, "_H_lo")]]
    hi  <- rd[[paste0(k, "_H_hi")]]
  ...
  gt(do.call(rbind, rows_med)) |>
    tab_header(title = "Median summary (companion to the table above)",
               subtitle = "Same detected reps - location columns are medians") |>
    fmt_number(columns = c(Med, Bias_med, SD_emp, SE_med, Width_med), decimals = 2) |>
    fmt_number(columns = Coverage, decimals = 2) |>
    cols_label(Med = "Median est.", Bias_med = "Median bias",
               SD_emp = "SD (emp)", SE_med = "Median SE",
               Coverage = "95% cov.", Width_med = "Median CI width") |>
    tab_footnote(footnote = "Same referents as the mean-based table above.")
}
```
...
# Covariate-frequency figures via the combine document's rule_covs() pattern
# (its enrichment block defines rule_covs() as a wrapper over the package
# internal .fs_rule_columns(); ported minimally here).  The recorder leaves
# `covs` NA by design (rule_covs is not a package export), so the covariates
# are recovered from sg_def post hoc.
if (nrow(rd)) {
