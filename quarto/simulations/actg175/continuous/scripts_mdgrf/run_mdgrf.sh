#!/bin/bash
# mdgrf Stage 2 runner (TASK_md_grf_2026-09-16 §2): transplant of scripts_mdsgnb20/run_mdsgnb20.sh
# with the GRF identifier knobs (FS_MD_METHOD=grf FS_MD_DMIN_GRF=30), the grf_ stem, the tag,
# the Gate 2 report name and scripts_mdgrf/gate2.R.  Everything else is the FS runner's.
# Launch from the repo root or anywhere (paths are resolved from this file):
#   MDSG_WORKERS=<W> MDSG_TIMEOUT=<s> MDSG_CEILING=<s> setsid nohup bash <dir>/scripts_mdgrf/run_mdgrf.sh > <dir>/logs_mdgrf/runner.log 2>&1 &
...
GREP=$QMD/REPORT_md_grf_gate2_${DATE}.md
...
render () {  # $1 cell  $2 stem  $3 label  $4 mode  $5 start  $6 nsims  $7 md  $8 n  -> sets WALL, PEAK
  ...
  local t0=$(date +%s)
  ( cd "$QMD" && \
    FS_MD_METHOD=grf FS_MD_DMIN_GRF=30 FS_MD_FOCUS=effMaxSG FS_MD_NBHD=0.20 FS_MD_FIELD_SCALEC=selected FS_MD_CI=field FS_MD_CAMPAIGN=$TAG FS_MD_FB=none \
...
for spec in "${CELLS[@]}"; do
  ...
  STEM=grf_effMaxSG_mr_field_${MDTOK}_knoise0_n${N}_nb20_${TAG}
  ...
  {
    [ -f "$GREP" ] || { echo "# REPORT — $TAG Gate 2 (per cell)"; echo; echo "Task: \`dev/tasks/TASK_md_grf_${DATE}.md\` §2.3. Every coverage figure of this campaign is conditional on the proposed family (GRF's family is generated from a fitted surface). Runner: \`$RQ/scripts_${TAG}/run_${TAG}.sh\`; checker: \`$RQ/scripts_${TAG}/gate2.R\`."; echo; }
    echo "## $CELL — md $MD, n $N"
    echo
    echo "- Stem: \`$STEM\`; HEAD before this cell's commit: $(git rev-parse --short HEAD); workers $MDSG_WORKERS; threads 1."
    echo "- Knobs: \`FS_MD_METHOD=grf FS_MD_DMIN_GRF=30 FS_MD_FOCUS=effMaxSG FS_MD_NBHD=0.20 FS_MD_FIELD_SCALEC=selected FS_MD_CI=field FS_MD_CAMPAIGN=$TAG FS_MD_FB=none FS_MD_MD=$MD FS_MD_N=$N FS_MD_WORKERS=$MDSG_WORKERS\`."
  ...
  PATHS=("$RQ/mr_md_harm/${STEM}_d5000" "$RQ/${STEM}_combine_1_2000.html" "$RQ/REPORT_md_grf_gate2_${DATE}.md" "$RQ/LOG_${TAG}_progress.txt")
