# ===== GATE 2, per cell, campaign mdgrf (GRF, effMaxSG, eps 0.20, dmin.grf 30, ACTG175 continuous / MD) =====
# Transplant of scripts_mdsgnb20/gate2.R (the committed Gate 2 of mdsgnb20: combine assertions,
# meta, finiteness, invariants, gamma, identities, same-draws in two directions, payload size),
# pointed at the GRF bundles (stem grf_effMaxSG_..._nb20_mdgrf) with TASK_md_grf_2026-09-16 §2.3:
#   - 2,000 rows with sim_id exactly 1-2000; batch files match the combined bundle on every column;
#   - meta as §1.6(b): identifier grf, dmin_grf 30, focus effMaxSG, band 0.20, the rule, ci_method
#     field, field_scale_complement selected, pkg_version 0.3.5, host pop-os;
...
#   - §1.6(b)'s field-s and recorder checks on every declared replicate;
#   - MR failures on declared replicates at most 40.
# The GRF additions of gbsg_020/scripts_dinamr/gate2G.R: admitted_n recorded prominently and
# n_family stated as the ENUMERATED POOL; n_cons_qual / band_n reported as structurally NA.
# EVERY coverage number this campaign produces is coverage of beta(H-hat) CONDITIONAL ON THE
# PROPOSED FAMILY: GRF's family is generated from a fitted surface, so the fixed-family
# condition does not hold.  Comparisons with FS are descriptive, not a contest.
# usage: Rscript gate2.R <md: 40|120|null> <n> <cell-label>; exit 1 on any failure.
QMD_DIR <- Sys.getenv("MDSG_DIR", unset = "..")
TAG     <- Sys.getenv("MDSG_TAG", unset = "mdgrf")
WORKERS <- Sys.getenv("MDSG_WORKERS", unset = NA)
HOST    <- Sys.getenv("MDSG_HOST", unset = "pop-os")
DMIN    <- as.numeric(Sys.getenv("MDSG_DMIN_GRF", unset = "30"))
TOL <- 1e-8
`%||%` <- function(a,b) if (is.null(a) || length(a)==0 || all(is.na(a))) b else a
mdtok <- function(md) if (md == "null") "mdnull" else sprintf("md%s", md)
fstem <- function(md, n) sprintf("grf_effMaxSG_mr_field_%s_knoise0_n%d_nb20_%s", mdtok(md), n, TAG)
...
gate2 <- function(md, n, cell) {
  ...
  mk("subgroup_method", "grf"); mk("dmin_grf", DMIN); mk("grf_selection", "frontier"); mk("grf_depth", 2L); mk("grf_select_statistic", "effect")
  ...
  # --- GRF identifier fields (gate2G.R's addition and relabelling) ---
  ...
  if (length(K)) cat(sprintf("  >> N_FAMILY (ENUMERATED POOL, not the qualified set): min %g  med %g  p90 %g  max %g\n",
      min(K), stats::median(K), stats::quantile(K,.9,names=FALSE), max(K)))
  P("admitted_n recorded on every row", all(is.finite(r$admitted_n)), sprintf("(%d of %d)", sum(is.finite(r$admitted_n)), nrow(r)))
  P("admitted_n >= 1 on every declared replicate", all(D$admitted_n >= 1L, na.rm = TRUE) && !anyNA(D$admitted_n))
  ...
  for (k in c("n_cons_qual","band_n"))
    cat(sprintf("  STRUCTURAL %-12s %s\n", k, if (all(is.na(r[[k]]))) "present, all-NA -- STRUCTURAL on GRF (no consistency screen), NOT a failure" else "POPULATED (unexpected on GRF)"))
  ...
  if (file.exists(fp)) { om <- readRDS(fp); fr <- om$results
    same_draws(r, fr, "mdgrf -> mdsgnb20", null_cell); same_draws(fr, r, "mdsgnb20 -> mdgrf", null_cell)
  ...
  cat(sprintf("  timing: fit_mr_secs mean %.1f median %.1f p90 %.1f max %.1f | id_secs (GRF fit) mean %.2f median %.2f max %.2f | fld_H_secs mean %.1f | fld_Hc_secs mean %.2f\n",
