# ===== GATE 2, per cell, campaign mddina (DINA, effMaxSG, eps 0.20, ACTG175 continuous / MD) =====
# Transplant of scripts_mdgrf/gate2.R (itself scripts_mdsgnb20/gate2.R: combine assertions, meta,
# finiteness, invariants, gamma, identities, same-draws in two directions, payload size), pointed at
# the DINA bundles (stem dina_effMaxSG_..._nb20_mddina) with TASK_md_dina_campaign_2026-09-17 Stage 2:
#   - 2,000 rows with sim_id exactly 1-2000; batch files match the combined bundle on every column;
#   - meta as §1.6(c): identifier dina with its arguments (dina_select_statistic effect, dina_args
#     list()), focus effMaxSG, band 0.20, the rule, ci_method field, field_scale_complement
#     selected, pkg_version 0.3.5, host pop-os;
#   - same draws as mdsgnb20, both directions: n_true identical and the oracle columns within 1e-8
#     on all 2,000 sim_ids (null cell: complement oracle only);
#   - §1.6(c)'s field-s and recorder checks on every declared replicate: the E2 fields
#     (dina_searched_n, dina_proposed_n, dina_tau_min, admitted_n) filled, every proposed
#     candidate at oriented tau-hat >= the effect threshold 30 (dina_tau_min >= 30), and
#     1 <= admitted_n <= dina_proposed_n;
#   - MR failures on declared replicates at most 40.
# The GRF checker's admitted_n / n_family block is kept with the DINA reading: admitted_n is read
# from DINA's selection object, so it exists on declared replicates only (NA on non-detections,
# structural); n_family is MR's family, DINA's proposed candidates with >= n.min members.
# n_cons_qual / band_n are reported as structurally NA (no consistency screen on DINA).
# EVERY coverage number this campaign produces is coverage of beta(H-hat) CONDITIONAL ON THE
# PROPOSED FAMILY: DINA's family is generated from a fitted surface, so the fixed-family
# condition does not hold.  Comparisons with FS and GRF are descriptive, not a contest.
# usage: Rscript gate2.R <md: 40|120|null> <n> <cell-label>; exit 1 on any failure.
QMD_DIR <- Sys.getenv("MDSG_DIR", unset = "..")
TAG     <- Sys.getenv("MDSG_TAG", unset = "mddina")
WORKERS <- Sys.getenv("MDSG_WORKERS", unset = NA)
HOST    <- Sys.getenv("MDSG_HOST", unset = "pop-os")
FLOOR   <- as.numeric(Sys.getenv("MDSG_FLOOR", unset = "30"))   # effect threshold = DINA proposal floor (oriented)
TOL <- 1e-8
`%||%` <- function(a,b) if (is.null(a) || length(a)==0 || all(is.na(a))) b else a
mdtok <- function(md) if (md == "null") "mdnull" else sprintf("md%s", md)
fstem <- function(md, n) sprintf("dina_effMaxSG_mr_field_%s_knoise0_n%d_nb20_%s", mdtok(md), n, TAG)
fdir  <- function(st) file.path(QMD_DIR, "mr_md_harm", paste0(st, "_d5000"))
fsp <- function(md, n) { st <- sprintf("fs_effMaxSG_mr_field_%s_knoise0_n%d_nb20_mdsgnb20", mdtok(md), n)
...
gate2 <- function(md, n, cell) {
  ...
  cat(sprintf("\n########## GATE 2 (%s): %s -- md %s, n %d ##########\n", TAG, cell, md, n))
  cat("  Every coverage figure of this campaign is conditional on the proposed family.\n")
  ...
  mk("subgroup_method", "dina"); mk("dina_select_statistic", "effect"); mk("dina_args", "list()")
  ...
  P("MR failures on declared replicates <= 40", mrfail <= 40L, sprintf("(%d)", mrfail))

  # --- DINA identifier fields (the GRF checker's block, read for DINA) ---
  A <- r$admitted_n[is.finite(r$admitted_n)]; K <- D$n_family[is.finite(D$n_family)]
  PR <- D$dina_proposed_n[is.finite(D$dina_proposed_n)]
  if (length(PR)) {
    q <- stats::quantile(PR, c(.25,.50,.75,.90), names = FALSE)
    cat(sprintf("  >> DINA_PROPOSED_N (>= floor): min %g  q25 %g  MED %g  q75 %g  p90 %g  max %g  (mean %.1f, n %d); searched %s\n",
        min(PR), q[1], q[2], q[3], q[4], max(PR), mean(PR), length(PR), paste(unique(range(D$dina_searched_n, na.rm = TRUE)), collapse = "-")))
  }
  if (length(A)) {
    q <- stats::quantile(A, c(.25,.50,.75,.90), names = FALSE)
    cat(sprintf("  >> ADMITTED_N (qualified)   : min %g  q25 %g  MED %g  q75 %g  p90 %g  max %g  (mean %.1f, n %d); == 0 on %d row(s)\n",
        min(A), q[1], q[2], q[3], q[4], max(A), mean(A), length(A), sum(A == 0L)))
  }
  if (length(K)) cat(sprintf("  >> N_FAMILY (MR's family: proposed candidates with >= n.min members): min %g  med %g  p90 %g  max %g\n",
      min(K), stats::median(K), stats::quantile(K,.9,names=FALSE), max(K)))
  E2 <- c("dina_searched_n", "dina_proposed_n", "dina_tau_min", "admitted_n")
  P("E2 fields filled on every declared replicate", all(E2 %in% names(r)) && all(vapply(E2, function(k) all(is.finite(D[[k]])), logical(1))),
    sprintf("(%s)", paste(sprintf("%s %d/%d", E2, vapply(E2, function(k) sum(is.finite(D[[k]])), 1L), nrow(D)), collapse = ", ")))
  cat(sprintf("  STRUCTURAL E2 fields on non-detections: all NA %s (DINA's selection object is absent when nothing is selected)\n",
      all(vapply(E2, function(k) all(is.na(r[[k]][!det])), logical(1)))))
  P(sprintf("every proposed candidate at oriented tau-hat >= %g (dina_tau_min, declared replicates)", FLOOR), all(D$dina_tau_min >= FLOOR),
    sprintf("(min %.4f)", min(D$dina_tau_min)))
  P("1 <= admitted_n <= dina_proposed_n on every declared replicate", all(D$admitted_n >= 1L & D$admitted_n <= D$dina_proposed_n))
  P("n_family finite on every declared replicate with a gate", all(is.finite(D$n_family[D$mr_ok %in% 1L])))
  P("p_hat_H and p_hat_sum recorded on declared replicates with a gate", all(is.finite(D$p_hat_H[D$mr_ok %in% 1L])) && all(is.finite(D$p_hat_sum[D$mr_ok %in% 1L])))
  P("p-hat validity (0<=p<=1, p_H<=sum)", all(D$p_hat_H >= 0 & D$p_hat_H <= 1, na.rm = TRUE) && all(D$p_hat_H <= D$p_hat_sum + 1e-12, na.rm = TRUE))
  for (k in c("n_cons_qual","band_n"))
    cat(sprintf("  STRUCTURAL %-12s %s\n", k, if (all(is.na(r[[k]]))) "present, all-NA -- STRUCTURAL on DINA (no consistency screen), NOT a failure" else "POPULATED (unexpected on DINA)"))
  wm <- r$warn_msg[!is.na(r$warn_msg)]
  cat(sprintf("  >> WARNINGS                 : %d of %d rows carry warn_msg; distinct messages: %s\n", length(wm), nrow(r),
  ...
  cat(sprintf("  >> p-hat(H): mean %.3f, share < 0.5: %.3f | CLASSIFICATION: sens %.4f ppv %.4f | mean |Hhat| (n_harm) %.1f\n",
      mean(Dm1$p_hat_H), mean(Dm1$p_hat_H < 0.5), mean(D$sens, na.rm = TRUE), mean(D$ppv, na.rm = TRUE), mean(D$n_harm, na.rm = TRUE)))

  fp <- fsp(md, n); P("mdsgnb20 comparator on disk", file.exists(fp), basename(fp))
  if (file.exists(fp)) { om <- readRDS(fp); fr <- om$results
    same_draws(r, fr, "mddina -> mdsgnb20", null_cell); same_draws(fr, r, "mdsgnb20 -> mddina", null_cell)
    cat(sprintf("  FS comparator (same cell): declaration %.4f | mean |Hhat| %.1f\n", mean(fr$detected %in% 1L), mean(fr$n_harm, na.rm = TRUE))) }

  for (fp2 in c(f1, f2, cf)) if (file.exists(fp2)) {
    sz <- file.size(fp2)
    P(sprintf("size <= 100 MB: %s", basename(fp2)), sz <= 100 * 1024^2, sprintf("(%d B)%s", sz, if (sz > 50 * 1024^2) "  ** FLAG: over 50 MB **" else ""))
  }
  cat(sprintf("  timing: fit_mr_secs mean %.1f median %.1f p90 %.1f max %.1f | id_secs (DINA fit) mean %.2f median %.2f max %.2f | fld_H_secs mean %.1f | fld_Hc_secs mean %.2f\n",
              mean(r$fit_mr_secs), median(r$fit_mr_secs), quantile(r$fit_mr_secs, .9, names = FALSE), max(r$fit_mr_secs),
              mean(r$id_secs, na.rm = TRUE), median(r$id_secs, na.rm = TRUE), max(r$id_secs, na.rm = TRUE),
