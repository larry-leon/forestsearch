...
# Scale check.  The oracle refits on the TRUE region, so its replicate spread
# estimates sigma[betahat_n(Q)] directly.  Printed HERE and not in run-batch,
# which carries #| output: false -- a check that cannot be read is not a check.
# Computed from rd rather than .payload so it also works in combine mode.
if (nrow(rd) > 1L && exists("dgm_scale")) {
