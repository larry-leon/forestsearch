...
# fs5_params.R
#
# Canonical FS parameter bundle "fs5" for ACTG175 binary harm simulations.
#
# Sourced by per-config qmds at:
#   quarto/simulations/actg175/actg175_binary_m1_harm_<focus>_fs5.qmd
#
# Provides one function: get_fs5_params(sg_focus, ...)
...
#
# -----------------------------------------------------------------------------
# Regime: fs5 — permissive search, broad candidate space, full cut retention.
#
...
#
# Key choices:
#   max_subgroups_search = 10  → broad candidate space (vs. fs5's 5)
...
#                            applied inside get_fs5_params())
#
# To create a sibling bundle "fs5" with different parameter choices:
#   1. Copy this file to fs5_params.R
#   2. Rename the function to get_fs5_params()
#   3. Edit the values that differ
#   4. Update the regime description above
#   5. Use clone_config.sh fs5 fs5 to spawn the parallel sg_focus qmds
# =============================================================================

#' Get FS parameters for the "fs5" bundle
#'
#' Returns the FS parameter list used by ACTG175 binary harm simulations
#' under the "fs5" regime. The bundle's \code{stop_threshold} is set to
...
get_fs5_params <- function(sg_focus,
                           outcome_name = "y_sim",
  ...
  list(
    ...
    max_subgroups_search      = 50,        # fs5: broad
    use_twostage              = TRUE,
    ...
    # stop_threshold: 0.90 is the fs5 bundle's value. Package requires
    # NULL when sg_focus uses neighborhood-based selection (hrMaxSG /
    # hrMinSG); applied here so qmds can call get_fs5_params() and use
