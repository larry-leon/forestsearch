...
# =============================================================================
# One-time setuop
# chmod +x quarto/simulations/actg175/binary/fs7_render_queue.sh
# fs7_render_queue.sh
#
# Serially render all fs7 ACTG simulation qmds. Continues past failures
...
#
# Usage:
#   ./quarto/simulations/actg175/binary/fs7_render_queue.sh
# =============================================================================

set -uo pipefail
shopt -s nullglob

QMDS=(quarto/simulations/actg175/binary/actg175_binary_m1_harm_*_fs7.qmd)

if [ "${#QMDS[@]}" -eq 0 ]; then
  echo "No fs7 qmds found.  Check the glob pattern and working directory."
  exit 1
fi

TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
LOG_FILE="quarto/simulations/fs7_render_${TIMESTAMP}.log"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "fs7 render queue starting: $(date)"
echo "Files: ${#QMDS[@]}"
echo "Log:   $LOG_FILE"
echo ""
