...
inspect the *Pareto frontier* on (HR, N) -- the set of candidate subgroups
...
```{r fs-pareto}
pareto <- fs$grp.consistency$out_sg$pareto_frontier

if (!is.null(pareto) && nrow(pareto) > 0) {
  # Display the M.<k> factor columns alongside key metrics
  m_cols <- grep("^M\\.", names(pareto), value = TRUE)
  display_cols <- intersect(
    c(m_cols, "N", "E", "hr", "Pcons"),
    names(pareto)
  )

  as.data.frame(pareto)[, display_cols, drop = FALSE] |>
    gt::gt() |>
    gt::tab_header(
      title = "Pareto frontier on (HR, N)",
      subtitle = sprintf(
        "%d non-dominated candidate(s) -- diagnostic only, not used for selection",
        nrow(pareto)
      )
    ) |>
    gt::fmt_number(columns = "hr",    decimals = 3) |>
    gt::fmt_number(columns = "Pcons", decimals = 3) |>
    gt::fmt_number(columns = c("N", "E"), decimals = 0)
} else {
  cat("No Pareto frontier available (no subgroups passed consistency).\n")
}
```
