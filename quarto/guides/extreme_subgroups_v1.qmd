...
```{r trial-km}
#| fig-cap: "Kaplan-Meier curves: one simulated trial. Solid = overall ITT; dashed = true harm subgroup; dotted = true no-harm subgroup."
library(survival)
...
# Unconditional tail probabilities for UB(HR).
# Denominated by valid (non-NA) simulations, consistent with pr_ub_ge1.
pr_ub_ge2 <- apply(sim_ubs, 2, function(x) mean(x >= 2.0, na.rm = TRUE))
pr_ub_ge3 <- apply(sim_ubs, 2, function(x) mean(x >= 3.0, na.rm = TRUE))

# Unconditional median UB(HR); ub_q[, 2] is the 50th percentile across sims.
mub_uncond <- ifelse(
  is.na(ub_q[, 2]), "-",
...
# Pull the raw 1st / median / 99th percentile triples for the included rows.
est_raw <- ub_q[include_highrisk, 2][ord_highrisk]
lo_raw  <- ub_q[include_highrisk, 1][ord_highrisk]
hi_raw  <- ub_q[include_highrisk, 3][ord_highrisk]

# Clamp whisker endpoints to xlim.  Without this, ggplot2's scale_x_*(limits)
# converts out-of-range values to NA and drops the *entire* linerange — the
# point still draws, leaving a misleading "point-only" row when the 1st or
# 99th percentile falls outside the axis.  The tail probabilities in the
# UB>=2 / UB>=3 columns already carry the truncated-tail information, so no
# information is lost by clipping the visual whisker.
xlim_ub <- c(0.30, 9.0)
lo_clip <- pmax(lo_raw, xlim_ub[1])
hi_clip <- pmin(hi_raw, xlim_ub[2])

# Flag rows where clipping occurred so the reader can tell visible-edge
# whiskers from genuinely-short ones.  Appended to mUB as a small arrow.
clip_lo <- !is.na(lo_raw) & lo_raw < xlim_ub[1]
clip_hi <- !is.na(hi_raw) & hi_raw > xlim_ub[2]
mub_lab <- mub_uncond[include_highrisk][ord_highrisk]
mub_lab <- ifelse(clip_hi, paste0(mub_lab, "\u2192"), mub_lab)   # right arrow
mub_lab <- ifelse(clip_lo, paste0("\u2190", mub_lab), mub_lab)   # left arrow
...
gg_forest(
  ...
  footnote    = paste0(
    ...
    n_sims_null, " simulated trials  |  ",
    "Whiskers clipped at axis limits (arrow in mUB column)  |  ",
    "Dotted: UB = 1.0;  Dashed: true HR = 0.70"
  ),
  point_size  = fp_point_size,
  line_size   = fp_line_size,
  base_size   = fp_base_size,
  widths      = c(3.5, 5, 1.1, 1.2, 1.2, 1.2)
)
```
...
# Inline HR tail summaries (analogous to pr_ub_ge2 / pr_ub_ge3).
# Pr(HR < 0.5)  -- chance of an apparently very large benefit by chance
# Pr(HR > 1.0)  -- chance the point estimate lands on the wrong side of no effect
pr_hr_lt050 <- apply(sim_hrs, 2, function(x) mean(x < 0.5, na.rm = TRUE))
pr_hr_gt1   <- apply(sim_hrs, 2, function(x) mean(x > 1.0, na.rm = TRUE))

# Unconditional median HR; hr_q[, 2] is the 50th percentile across sims.
mhr_uncond <- ifelse(
  is.na(hr_q[, 2]), "-",
  as.character(round(hr_q[, 2], 2))
)

# Pull raw quantiles for the included rows.
est_raw_hr <- hr_q[include_highrisk, 2][ord_highrisk]
lo_raw_hr  <- hr_q[include_highrisk, 1][ord_highrisk]
hi_raw_hr  <- hr_q[include_highrisk, 3][ord_highrisk]

# Clamp whisker endpoints to xlim — same reasoning as in ub-forest-highrisk:
# without this, percentiles outside the axis become NA and the entire
# linerange is dropped, leaving a misleading "point-only" row.  Especially
# severe for very small subgroups (random15, random20) whose 1st percentile
# falls well below xlim[1] and 99th percentile well above xlim[2].
xlim_hr <- c(0.15, 3.5)
lo_clip_hr <- pmax(lo_raw_hr, xlim_hr[1])
hi_clip_hr <- pmin(hi_raw_hr, xlim_hr[2])

# Mark clipped rows with arrows in the mHR column.
clip_lo_hr <- !is.na(lo_raw_hr) & lo_raw_hr < xlim_hr[1]
clip_hi_hr <- !is.na(hi_raw_hr) & hi_raw_hr > xlim_hr[2]
mhr_lab <- mhr_uncond[include_highrisk][ord_highrisk]
mhr_lab <- ifelse(clip_hi_hr, paste0(mhr_lab, "\u2192"), mhr_lab)
mhr_lab <- ifelse(clip_lo_hr, paste0("\u2190", mhr_lab), mhr_lab)
...
gg_forest(
  ...
  footnote    = paste0(
    ...
    n_sims_null, " simulated trials  |  ",
    "Whiskers clipped at axis limits (arrow in mHR column)  |  ",
