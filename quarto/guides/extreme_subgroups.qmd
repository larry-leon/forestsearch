...
is independent and uses iteration-local seeds (`seed = ss` for
`simulate_from_dgm()`, `set.seed(ss + 1e6L)` for the random subgroup
re-draw) — so bit-for-bit reproducibility is preserved regardless of
execution order.  We use `doFuture` with a `multisession` plan (portable
across Windows / macOS / Linux) and target **at least 90% of available
cores**, so a 5,000-sim run scales close to the number of physical
cores available.
...
# Each iteration is independent and self-seeded.  `.options.future`:
#   seed = NULL     → declare we manage RNG ourselves (suppresses
#                     warnings; per-iteration set.seed() calls below
#                     guarantee bit-for-bit reproducibility)
#   packages = ...  → load survival + forestsearch on each worker
#                     (multisession workers start with a clean session)
sim_results <- foreach::foreach(
  ss = seq_len(n_sims_null),
  .options.future = list(
    seed     = NULL,
  ...
  # Per-iteration storage: length-n_sg numerics, recombined to matrices below
...
# ── Recombine per-iteration results into the three matrices ──────────────
...
# Release workers (memory hygiene; not strictly required but good citizenship)
...
in `n_sims_null`; the DGM and censoring-calibration steps are one-time
...
t_per_trial   <- t_sims / n_sims_null
...
timing_df <- data.frame(
  Step = c(
    ...
    sprintf("Simulation loop (%d trials \u00d7 %d subgroups, %d workers)",
            n_sims_null, length(subgroups), n_workers),
    "Total"
  ),
...
knitr::kable(timing_df,
             align   = c("l", "r", "r"),
             caption = sprintf(
               "Timing for n_sims_null = %d, parallel across %d of %d cores.",
               n_sims_null, n_workers, n_cores_total))
