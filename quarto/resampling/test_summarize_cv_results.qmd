...
params:
  ...
  force_rerun: false     # TRUE: ignore checkpoint and re-run CV.
---
...
**Checkpoint behaviour.** The CV run is saved to
`test_summarize_cv_checkpoint_quick.rds` or
`test_summarize_cv_checkpoint_full.rds` in the working
directory (suffix depends on `params$quick_mode`).  If that
file exists and `params$force_rerun` is `FALSE`, the CV is
skipped.  Stale checkpoints that predate Phase B (missing the
`pconsistency` column) are detected automatically and refreshed.
...
if (isTRUE(params$quick_mode)) {
  Ksims_cv  <- 10L
  Kfolds_cv <- 10L
  ckpt_path <- "test_summarize_cv_checkpoint_quick.rds"
  cat("Mode: quick (sims = 10).  Set quick_mode = FALSE for sims = 100.\n")
} else {
  Ksims_cv  <- 100L
  Kfolds_cv <- 10L
  ckpt_path <- "test_summarize_cv_checkpoint_full.rds"
...
if (file.exists(ckpt_path) && !isTRUE(params$force_rerun)) {
  message(sprintf("Loading CV from checkpoint: %s", ckpt_path))
  tf <- readRDS(ckpt_path)

  # Phase B staleness check: older checkpoints lack one or more of the
  # new diagnostic columns.  Rerun rather than silently failing Test 7.
  required_phase_b <- c("pconsistency", "training_fs_hr",
                        "n_candidates_evaluated")
  missing_phase_b <- setdiff(required_phase_b, names(tf$fold_summary))
  if (length(missing_phase_b) > 0L) {
    message(sprintf(paste0(
      "Checkpoint %s predates full Phase B (missing: %s). ",
      "Re-running CV..."), ckpt_path,
      paste(missing_phase_b, collapse = ", ")))
    tf <- .run_cv()
    saveRDS(tf, ckpt_path)
...
expected_metrics <- c(
  ...
  "Training-fold HR (identifying folds)",
...
for (spec in list(
  ...
  list(metric = "Training-fold HR (identifying folds)",
       col    = "training_fs_hr",
       filt   = function(x) !is.na(x)),
