...
# (multiplier resampling; the `debias_gate` surface prior to the MR rename)
# =============================================================================
# This file is deliberately written against the CURRENT names so that it can be
# committed BEFORE the gate/tier -> FB/MR rename and re-run after it: if the
# rename is behaviour-preserving, only the identifiers in this file change.
#
# What is pinned here:
...
# gated fits are built once at file scope rather than per expectation.
.mri_args <- function(...) {
...
test_that("the de-biasing step is off by default and returns NULL when off", {
  expect_identical(formals(forestsearch)$debias_gate, FALSE)
  expect_identical(formals(forestsearch)$debias_gate_args, quote(list()))

  fs <- .run_fs_capture(.mri_data, .mri_args())$value

  # The slot must EXIST (downstream consumers index it) and be NULL.
  expect_true("debias_gate" %in% names(fs))
  expect_null(fs$debias_gate)
  expect_true("harm_flag_debiased" %in% names(fs))
  expect_true(is.na(fs$harm_flag_debiased))
...
.mri_fit <- local({
  r <- .run_fs_capture(.mri_data,
                       .mri_args(debias_gate = TRUE,
                                 debias_gate_args = .mri_draws))
  r
})
...
test_that("enabled on the Cox/consistency path it returns the documented fields", {
  ...
  expect_true(all(c("selected_index", "selected_label", "measure", "log_scale",
                    ...
                    "fixed_bias", "selection_rate", "complement", "gate",
  ...
  expect_true(all(c("t_gate", "type", "reselection", "selection_rule",
                    "multiplier", "draws") %in% names(g$gate)))
  expect_identical(g$gate$type, "point")
  expect_identical(g$gate$draws, 200L)
  expect_identical(g$gate$multiplier, "poisson")
  # Ratio measure -> the near-null default threshold is 1.
  expect_equal(g$gate$t_gate, 1)
  ...
  expect_identical(fs$harm_flag_debiased, g$harm_flag)
})
...
test_that("the complement is de-biased by default from forestsearch()", {
  ...
  # complement block is populated in practice even though fs_debias_gate()'s
  # own formal default is FALSE.
  cc <- .mri_fit$value$debias_gate$complement
...
test_that("enabling the de-biasing step does not alter identification", {
  ...
  # into selection. Same data, same seed, gate on vs off.
...
test_that("no subgroup identified gives a clean skip, NA flag, and no error", {
  ...
  r <- .run_fs_capture(
    ...
    .mri_args(hr.threshold = 3.0, pconsistency.threshold = 0.99,
              debias_gate = TRUE, debias_gate_args = .mri_draws))
  fs <- r$value
  ...
  expect_false(any(grepl("debias_gate|de-biased gate", r$warnings,
                         ignore.case = TRUE)))
})
...
test_that("both harm-confirmation rules run and can disagree", {
  ...
  mk <- function(rule) {
    .run_fs_capture(
      .mri_data,
      .mri_args(debias_gate = TRUE,
                debias_gate_args = utils::modifyList(
                  .mri_draws, list(gate = rule, t_gate = t_mid))))$value$debias_gate
  }
  gp <- mk("point")
  gc <- mk("ci")
  ...
  expect_identical(gp$gate$type, "point")
  expect_identical(gc$gate$type, "ci")
  expect_equal(gp$gate$t_gate, t_mid)
  expect_equal(gc$gate$t_gate, t_mid)
...
test_that("sg_focus maps to the re-selection rule per engine, for all eleven foci", {
  ...
  # Every rule the mapping can produce must be one fs_debias_gate() accepts.
  accepted <- eval(formals(fs_debias_gate)$reselection)
...
test_that("the rule reaching fs_debias_gate() is the one sg_focus implies", {
  ...
  r <- .run_fs_capture(
    .mri_data,
    .mri_args(sg_focus = "maxeffCons", debias_gate = TRUE,
              debias_gate_args = .mri_draws))$value
  skip_if(is.null(r$debias_gate), "de-biasing step did not run under maxeffCons")
  expect_identical(r$debias_gate$gate$reselection, "maxeff")
})
...
test_that("GLM outcomes silently skip the step under the default consistency_method", {
  ...
  expect_null(r_split$value$debias_gate)             # ... yet nothing ran
  expect_true(is.na(r_split$value$harm_flag_debiased))
  expect_false(any(grepl("debias_gate|de-biased gate", r_split$warnings,
  ...
  expect_false(is.null(r_res$value$debias_gate))
  expect_false(is.na(r_res$value$harm_flag_debiased))
})
