...
with(env, {
  ...
  cat("GBSG table: c0 | kappa05 | kappa10 | pstar_impl05 | fw(p*0.90) | n_adm_cal05 | q90 q95 q99\n")
  for (i in seq_len(nrow(t05))) cat(t05$c0[i], "|", f(t05$kappa_hat[i]), "|", f(t10$kappa_hat[i]), "|", f(t05$pstar_implied[i]),
  ...
  tb <- fs_declaration_calibration(mr, c0 = .c0_5)$c0$table
  ...
  fw <- mean(m > qnorm(0.95)); k <- quantile(m, 0.95, type = 1, names = FALSE)
