...
3.  **Running simulated trials**: drawing `r nsims_null` under null (uniform benefit) and `r nsims_alt` under alternative (HTEs)
4. **Run Analyses**: Apply ForestSearch (and optionally GRF) to each dataset
5. **Summarize Results**: Aggregate operating characteristics across simulations
...
The DGM defines the ground truth for the simulation. Building on any covariate template dataset, `generate_aft_dgm_flex()` fits an
...
- **Hypothesis**: `model = "alt"` introduces treatment effect heterogeneity
  ...
  uniform treatment effect (for type-I error evaluation).
- **Effect size**: The `k_inter` parameter scales the treatment-by-covariate
  interaction. Use `find_k_inter_for_target_hr()` to find the value that
  achieves a target HR in the harm subgroup.
- **Subgroup definition**: Specified via `subgroup_vars` and `subgroup_cuts`.
  Here H = {z1 == 1 AND z3 == 1} (low ER and premenopausal), covering roughly
  13% of the super-population.
- **Censoring**: Fitted from observed data by default (`select_censoring = TRUE`);
  adjusted at simulation time via `cens_adjust` in `simulate_from_dgm()`.
...
`run_simulation_analysis()` internally calls `simulate_from_dgm()` and normalises
all output to a single **canonical underscore column convention**:
`y_sim`, `event_sim`, `flag_harm`, `loghr_po`, `theta_0`, `theta_1`.
...
- Applies censoring with optional log-scale adjustment (`cens_adjust`) and
  administrative censoring at `analysis_time`.
- Carries forward the true subgroup indicator `flag_harm` and individual
  `loghr_po` for evaluation.

Simulator arguments are passed via the `sim_args` list in
...
```r
# Step 1: Create DGM and calibrate
cal     <- find_k_inter_for_target_hr(target_hr_harm = 2.0, data = gbsg_data,
             continuous_vars = dgm_cont_vars, factor_vars = dgm_factor_vars,
             outcome_var = "y", event_var = "event", treatment_var = "treat",
             subgroup_vars = c("z1", "z3"),
             subgroup_cuts = list(z1 = list(type = "multiple", values = 1),
                                  z3 = list(type = "multiple", values = 1)))
dgm     <- cal$dgm
...
results <- foreach(sim = 1:500, .combine = rbind) %dofuture% {
  run_simulation_analysis(sim_id = sim, dgm = dgm, n_sample = 700,
                          ...
                          sim_args = list(analysis_time = 84, cens_adjust = log(1.5),
                                          max_entry = 0))
}
...
(`survival::gbsg`) as a covariate template. The dataset is first pre-processed to
create the same binary prognostic variables used in Leon et al. (2024), then passed
to `generate_aft_dgm_flex()`.
...
# ── Load and derive variables (matching Leon et al. 2024 DGM structure) ─────
gbsg_data        <- survival::gbsg
gbsg_data$y      <- gbsg_data$rfstime / DAYS_PER_MONTH  # days -> months
gbsg_data$event  <- as.integer(gbsg_data$status == 1)
gbsg_data$treat  <- gbsg_data$hormon
gbsg_data$id     <- seq_len(nrow(gbsg_data))

# Subgroup-defining binary variables
er_threshold     <- quantile(gbsg_data$er, 0.25)
gbsg_data$z1     <- as.integer(gbsg_data$er    <= er_threshold)  # low ER
gbsg_data$z2     <- as.integer(gbsg_data$age   >  median(gbsg_data$age))
gbsg_data$z3     <- as.integer(gbsg_data$meno  == 0)             # premenopausal
gbsg_data$z4     <- as.integer(gbsg_data$pgr   >  median(gbsg_data$pgr))
gbsg_data$z5     <- as.integer(gbsg_data$nodes >  median(gbsg_data$nodes))
gbsg_data$grade3 <- as.integer(gbsg_data$grade == 3)
# size kept on original scale (mm); included as a confounder for analysis

# ── DGM variable lists (reused across all generate_aft_dgm_flex calls) ───────
dgm_cont_vars <- c("z1", "z2", "z3", "z4", "z5", "size")
dgm_factor_vars <- character(0)          # z-vars are already numeric (0/1 or 1-4)

# Subgroup: H = {z1 == 1 & z3 == 1}  (low ER AND premenopausal)
dgm_subgroup_vars <- c("z1", "z3")
dgm_subgroup_cuts <- list(
  z1 = list(type = "multiple", values = 1),
  z3 = list(type = "multiple", values = 1)
)

cat("GBSG data prepared:\n")
cat("  N =", nrow(gbsg_data), "\n")
cat("  Events =", sum(gbsg_data$event),
    "(", round(100 * mean(gbsg_data$event), 1), "%)\n")
cat("  Subgroup H size (z1 & z3 == 1):",
    sum(gbsg_data$z1 == 1 & gbsg_data$z3 == 1),
    "(", round(100 * mean(gbsg_data$z1 == 1 & gbsg_data$z3 == 1), 1), "%)\n")
cat("  ER threshold (Q25):", round(er_threshold, 1), "\n")
```
...
# Create DGM with heterogeneous treatment effect via generate_aft_dgm_flex()
dgm_alt <- generate_aft_dgm_flex(
  ...
  model           = "alt",
  k_treat         = 1.0,
  k_inter         = 2.0,    # Starting value; calibrated below
  n_super         = 5000,
  cens_type       = "weibull",
  seed            = 8316951,
  verbose         = TRUE
...
# print.aft_dgm_flex shows a compact summary (suppresses df_super)
print(dgm_alt)

timings$dgm_creation <- proc.time()[3] - t0
```

### Accessing Hazard Ratios

```{r access-hrs}
# New unified access via $hazard_ratios list
cat("Cox-based HRs:\n")
cat("  HR(H):",       round(dgm_alt$hazard_ratios$harm_subgroup,    4), "\n")
cat("  HR(Hc):",      round(dgm_alt$hazard_ratios$no_harm_subgroup, 4), "\n")
cat("  HR(overall):", round(dgm_alt$hazard_ratios$overall,          4), "\n")

cat("\nAverage Hazard Ratios (from loghr_po):\n")
cat("  AHR(H):",       round(dgm_alt$hazard_ratios$AHR_harm,    4), "\n")
cat("  AHR(Hc):",      round(dgm_alt$hazard_ratios$AHR_no_harm, 4), "\n")
cat("  AHR(overall):", round(dgm_alt$hazard_ratios$AHR,         4), "\n")

# Controlled Direct Effects (CDE) — populated by compute_dgm_cde() above
...
# aft_dgm_flex stores super-population in $df_super
df_super <- dgm_alt$df_super
...
cat("  dgm$AHR =", round(dgm_alt$hazard_ratios$AHR, 4), "\n")
...
Use `find_k_inter_for_target_hr()` to find the interaction parameter that
achieves a target HR in the harm subgroup. This function wraps
`generate_aft_dgm_flex()` with `uniroot` and returns the calibrated DGM
directly — no separate `create_dgm` call needed.

```{r calibrate-k-inter-cox}
t0 <- proc.time()[3]

# Find k_inter for Cox-based HR = 2.0 in the harm subgroup.
# Returns a list: $k_inter, $achieved_hr_harm, $dgm (ready to use).
cal_cox <- find_k_inter_for_target_hr(
  target_hr_harm  = 2.0,
  data            = gbsg_data,
  continuous_vars = dgm_cont_vars,
  factor_vars     = dgm_factor_vars,
...
dgm_calibrated_cox <- cal_cox$dgm
dgm_calibrated_cox <- compute_dgm_cde(dgm_calibrated_cox)

cat("\nVerification (Cox-based):\n")
cat("Achieved HR(H):",  round(dgm_calibrated_cox$hazard_ratios$harm_subgroup,    3), "\n")
cat("HR(Hc):",          round(dgm_calibrated_cox$hazard_ratios$no_harm_subgroup, 3), "\n")
cat("Overall HR:",      round(dgm_calibrated_cox$hazard_ratios$overall,          3), "\n")

timings$calibrate_cox <- proc.time()[3] - t0
```

### Cox HR and AHR at Calibrated k_inter

`find_k_inter_for_target_hr()` calibrates to Cox-based HR in the harm subgroup.
Once the optimal `k_inter` is found, both the Cox HR and the AHR (average hazard
ratio via `loghr_po`) can be read from the resulting DGM. These two metrics
diverge slightly due to Jensen's inequality.

```{r calibrate-k-inter-ahr}
# The calibrated DGM already contains both Cox HR and AHR.
# No separate AHR-targeted calibration is needed; just read both metrics
# from dgm_calibrated_cox.

dgm_calibrated <- dgm_calibrated_cox

cat("Cox HR and AHR metrics at calibrated k_inter:\n")
cat(sprintf("  k_inter (calibrated):  %.4f\n", cal_cox$k_inter))
cat(sprintf("  Cox HR(H):             %.4f\n", dgm_calibrated$hazard_ratios$harm_subgroup))
cat(sprintf("  Cox HR(Hc):            %.4f\n", dgm_calibrated$hazard_ratios$no_harm_subgroup))
cat(sprintf("  AHR(H):                %.4f\n", dgm_calibrated$hazard_ratios$AHR_harm))
cat(sprintf("  AHR(Hc):               %.4f\n", dgm_calibrated$hazard_ratios$AHR_no_harm))
cat(sprintf("  AHR(overall):          %.4f\n", dgm_calibrated$hazard_ratios$AHR))
cat(sprintf("  Note: Cox HR(H) ~ %.3f (target); AHR(H) differs due to Jensen's inequality\n",
            cal_cox$target_hr_harm))
```

## Validating k_inter Effect on Heterogeneity

`validate_k_inter_effect()` sweeps a range of `k_inter` values and confirms
that `k_inter = 0` gives HR(H)/HR(Hc) ≈ 1 (no heterogeneity) and that
increasing `k_inter` drives the ratio in the expected direction.  Run once
after data preparation to sanity-check the subgroup definition before
committing to a full calibration.
...
For Type I error evaluation, create a DGM with no treatment effect heterogeneity:
...
dgm_null <- generate_aft_dgm_flex(
  ...
  continuous_vars = dgm_cont_vars,
  factor_vars     = dgm_factor_vars,
  subgroup_vars   = dgm_subgroup_vars,   # H is still DEFINED (z1=1 & z3=1) ...
  subgroup_cuts   = dgm_subgroup_cuts,   # ... just no differential treatment effect
  model           = "null",              # interaction term omitted => uniform TE
  k_treat         = 1.0,
  n_super         = 5000,
  seed            = 8316951,
  verbose         = TRUE
)
dgm_null <- compute_dgm_cde(dgm_null)

# Under null, HR(H) == HR(Hc) == HR(overall): confirms uniform treatment effect.
# flag_harm is still assigned in simulated data so sens/spec/ppv are meaningful.
cat("\nNull hypothesis HRs:\n")
cat("Overall HR:  ", round(dgm_null$hazard_ratios$overall,          3), "\n")
cat("HR(H):       ", round(dgm_null$hazard_ratios$harm_subgroup,    3), "\n")
cat("HR(Hc):      ", round(dgm_null$hazard_ratios$no_harm_subgroup, 3), "\n")
cat("AHR(overall):", round(dgm_null$hazard_ratios$AHR,              3), "\n")
...
Use `simulate_from_dgm()` to generate a single simulated trial directly:

```{r simulate-single}
# Use the Cox-calibrated DGM going forward
dgm_calibrated <- dgm_calibrated_cox

# Direct call for illustration; run_simulation_analysis() handles this
# internally with the same arguments via sim_args.
sim_data <- simulate_from_dgm(
  ...
  cens_adjust   = log(1.5)   # Censoring adjustment (log scale)
)
# simulate_from_dgm() already uses canonical underscore column names:
# y_sim, event_sim, flag_harm, loghr_po, theta_0, theta_1
...
# Quick survival analysis using canonical column names
fit_itt <- coxph(Surv(y_sim, event_sim) ~ treat, data = sim_data)
...
if ("loghr_po" %in% names(sim_data)) {
  ...
  # Compute AHR in simulated data by subgroup using canonical flag_harm column
...
results_alt <- foreach(
  ...
  run_simulation_analysis(
    ...
    cox_formula_adj = survival::Surv(y_sim, event_sim) ~ treat + z1 + z2 + z3,
    n_add_noise = 0L,
    run_fs = TRUE,
    run_fs_grf = FALSE,
    run_grf = TRUE,
...
if (length(ahr_cols) > 0) {
  ...
  if (nrow(results_found) > 0 && "ahr.H.hat" %in% names(results_found)) {
    ...
    cat("  True AHR(H):", round(dgm_calibrated$hazard_ratios$AHR_harm, 3), "\n")
    cat("  True AHR(Hc):", round(dgm_calibrated$hazard_ratios$AHR_no_harm, 3), "\n")
  }
}
...
if (length(ahr_cols) > 0) {
  ...
  if (nrow(results_found) > 0 && "ahr.H.hat" %in% names(results_found)) {
    ...
    cat("  Mean AHR(H) estimated:", round(mean(results_found$ahr.H.hat, na.rm = TRUE), 3), "\n")
    cat("  Mean AHR(Hc) estimated:", round(mean(results_found$ahr.Hc.hat, na.rm = TRUE), 3), "\n")
    cat("  True AHR(H):", round(dgm_null$hazard_ratios$AHR_harm, 3), "\n")
    cat("  True AHR(Hc):", round(dgm_null$hazard_ratios$AHR_no_harm, 3), "\n")
  }
}
...
build_classification_table(
  ...
  title = "Subgroup Identification and Classification Rates",
    # n_sims drives the subtitle; use the common count (alt and null are equal here).
  # If they differ, pass NULL and add a manual subtitle instead.
  n_sims = sim_config_null$n_sims
)
```
...
df_super_calib <- dgm_calibrated$df_super
flag_col <- "flag_harm"
n_sg_expected <- sim_config_alt$n_sample * mean(df_super_calib[[flag_col]])
...
cat("True HR in H:", round(dgm_calibrated$hazard_ratios$harm_subgroup, 3), "\n")
...
# True H is dgm_calibrated$hazard_ratios$harm_subgroup
...
prob_detect_null <- compute_detection_probability(
 theta = dgm_null$hazard_ratios$overall,
...
points(
  x = rep(dgm_calibrated$hazard_ratios$harm_subgroup, length(empirical_rates)),
  y = empirical_rates,
...
abline(v = dgm_calibrated$hazard_ratios$harm_subgroup, lty = 2, col = "blue", lwd = 1)

# Legend for empirical points
legend(
  "topleft",
  legend = c(
    sprintf("H true = %.2f", dgm_calibrated$hazard_ratios$harm_subgroup),
...
save_simulation_results(
  ...
  ahr_metrics = list(
    AHR_H_true = dgm_calibrated$hazard_ratios$AHR_harm,
    AHR_Hc_true = dgm_calibrated$hazard_ratios$AHR_no_harm,
    AHR = dgm_calibrated$hazard_ratios$AHR
  )
)
...
# --- Prepare GBSG data ---
gbsg_data        <- survival::gbsg
gbsg_data$y      <- gbsg_data$rfstime / DAYS_PER_MONTH
gbsg_data$event  <- as.integer(gbsg_data$status == 1)
gbsg_data$treat  <- gbsg_data$hormon
er_q25           <- quantile(gbsg_data$er, 0.25)
gbsg_data$z1     <- as.integer(gbsg_data$er   <= er_q25)
gbsg_data$z2     <- as.integer(gbsg_data$age  >  median(gbsg_data$age))
gbsg_data$z3     <- as.integer(gbsg_data$meno == 0)
gbsg_data$z4     <- as.integer(gbsg_data$pgr  >  median(gbsg_data$pgr))
gbsg_data$z5     <- as.integer(gbsg_data$nodes > median(gbsg_data$nodes))
gbsg_data$grade3 <- as.integer(gbsg_data$grade == 3)

cont_vars     <- c("z1", "z2", "z3", "z4", "z5", "size")
factor_vars   <- character(0)
subgroup_vars <- c("z1", "z3")
subgroup_cuts <- list(z1 = list(type = "multiple", values = 1),
                      z3 = list(type = "multiple", values = 1))

# --- Create and calibrate DGM ---
cal <- find_k_inter_for_target_hr(
...
dgm <- compute_dgm_cde(cal$dgm)

cat("\nDGM Summary:\n")
cat("  Cox HR(H):",  round(dgm$hazard_ratios$harm_subgroup,    3), "\n")
cat("  AHR(H):",     round(dgm$hazard_ratios$AHR_harm,         3), "\n")
cat("  Cox HR(Hc):", round(dgm$hazard_ratios$no_harm_subgroup, 3), "\n")
cat("  AHR(Hc):",    round(dgm$hazard_ratios$AHR_no_harm,      3), "\n")
...
results <- foreach(
  ...
  run_simulation_analysis(
    ...
    dgm              = dgm,
    n_sample         = N_SAMPLE,
    sim_args         = list(analysis_time = 60, cens_adjust = log(1.5)),
...
if (nrow(results_found) > 0 && "ahr.H.hat" %in% names(results_found)) {
  cat("\nAHR Estimates:\n")
  cat("  True AHR(H):", round(dgm$hazard_ratios$AHR_harm, 3), "\n")
...
| Step | Function | Purpose |
|------|----------|------|
| 1. Prep data | (user) | Derive z1-z5 variables from raw data |
| 2. Create DGM | `generate_aft_dgm_flex()` | Define data generating mechanism |
| 3. Calibrate | `find_k_inter_for_target_hr()` | Achieve target subgroup HR |
| 4. Simulate & Analyse | `run_simulation_analysis()` | Simulate trials + run ForestSearch/GRF |
...
|------------------|-------------------------------|------------------------------------------|
| `theta_H_true`   | `dgm$hazard_ratios$harm_subgroup`              | Cox-based HR in the true harm subgroup   |
| `theta_Hc_true`  | `dgm$hazard_ratios$no_harm_subgroup`             | Cox-based HR in the true complement      |
...
```r
hr_overall  <- get_dgm_hr(dgm, "hr_overall")  # hazard_ratios$overall for aft_dgm_flex
