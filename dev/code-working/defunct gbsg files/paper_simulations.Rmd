...
# Same call, different target metric: AHR instead of Cox HR.
# Achieve AHR(H) = 2.0 by searching over k_inter with uniroot.
# (AHR and Cox-HR diverge slightly due to Jensen's inequality.)
cal_ahr <- find_k_inter_for_target_hr(
  target_hr_harm  = 2.0,
  data            = gbsg_data,
  continuous_vars = dgm_cont_vars,
  factor_vars     = dgm_factor_vars,
...
dgm_calibrated_ahr <- cal_ahr$dgm
dgm_calibrated_ahr <- compute_dgm_cde(dgm_calibrated_ahr)

cat("\nVerification (AHR-based):\n")
cat("Achieved AHR(H):", round(dgm_calibrated_ahr$hazard_ratios$AHR_harm,    3), "\n")
cat("AHR(Hc):",         round(dgm_calibrated_ahr$hazard_ratios$AHR_no_harm, 3), "\n")
cat("Overall AHR:",     round(dgm_calibrated_ahr$hazard_ratios$AHR,         3), "\n")
...
cat(sprintf("%-20s %-12.4f %-12.4f\n", "HR(H)",
            dgm_calibrated_cox$hazard_ratios$harm_subgroup,
            dgm_calibrated_ahr$hazard_ratios$harm_subgroup))
cat(sprintf("%-20s %-12.4f %-12.4f\n", "AHR(H)",
            dgm_calibrated_cox$hazard_ratios$AHR_harm,
            dgm_calibrated_ahr$hazard_ratios$AHR_harm))
```
