...
generates a super-population of potential outcomes. The legacy
`create_gbsg_dgm()` (GBSG-specific) is still supported and fully
backward-compatible.
...
`run_simulation_analysis()` internally calls `dispatch_simulate()` (unexported),
which
routes to the correct simulator (`simulate_from_dgm()` for `aft_dgm_flex`
objects, `simulate_from_gbsg_dgm()` for `gbsg_dgm` objects) and normalises
...
- Applies censoring with optional adjustment (`cens_adjust` for
  `aft_dgm_flex`, or `muC_adj` for legacy `gbsg_dgm`) and administrative
  censoring at `analysis_time` / `max_follow`.
...
`run_simulation_analysis()`. For `aft_dgm_flex` DGMs, pass
`sim_args = list(analysis_time = 84, cens_adjust = 0.2)`. For backward
compatibility with `gbsg_dgm`, pass
`sim_args = list(max_follow = 84, muC_adj = log(1.5))`.
...
The super-population data (`dgm$df_super` for `aft_dgm_flex`, or
`dgm$df_super_rand` for `gbsg_dgm`) now contains:
...
# Access super-population — works with both DGM types:
# aft_dgm_flex stores it as $df_super; gbsg_dgm stores it as $df_super_rand
df_super <- if (!is.null(dgm_alt$df_super)) dgm_alt$df_super else dgm_alt$df_super_rand
...
# Simulate a single trial directly.
# Note: run_simulation_analysis() handles dispatch internally for both
# gbsg_dgm and aft_dgm_flex objects; this direct call is for illustration.
...
# Normalise to canonical underscore column names used throughout
sim_data$y_sim     <- sim_data$y.sim
sim_data$event_sim <- sim_data$event.sim
sim_data$flag_harm <- sim_data$flag.harm

# Examine the data — columns are now in canonical underscore convention:
# y_sim, event_sim, flag_harm, loghr_po, theta_0, theta_1
cat("Simulated trial:\n")
cat("  N =", nrow(sim_data), "\n")
...
df_super_calib <- if (!is.null(dgm_calibrated$df_super)) dgm_calibrated$df_super else dgm_calibrated$df_super_rand
flag_col <- if ("flag_harm" %in% names(df_super_calib)) "flag_harm" else "flag.harm"
...
# --- Create DGM ---
# Option 1: Calibrate to Cox-based HR (gbsg_dgm, backward-compatible path)
...
# Option 2: General path — use generate_aft_dgm_flex() on your own data
# dgm <- generate_aft_dgm_flex(data = my_data, ...)
...
# --- Run simulations ---
# confounders_base must always be supplied explicitly
...
results <- foreach(
  ...
  run_simulation_analysis(
    ...
    dgm      = dgm,
    n_sample = N_SAMPLE,
    # For gbsg_dgm: use max_follow / muC_adj in sim_args
    # For aft_dgm_flex: use analysis_time / cens_adjust in sim_args
    sim_args = list(max_follow = 60, muC_adj = log(1.5)),
...
|------|----------|---------|
| 1. Create DGM | `generate_aft_dgm_flex()` / `create_gbsg_dgm()` | Define data generating mechanism |
...
| 4. Simulate | `run_simulation_analysis()` | Simulate + analyse (dispatches internally to correct simulator) |
