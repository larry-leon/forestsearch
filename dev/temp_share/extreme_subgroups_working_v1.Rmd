...
# Set to 5000 for publication-quality results
n_sims_null <- 20L   # set to 5000 for publication-quality results
```
...
```{r simulation-flow-diagram, fig.cap = "Simulation workflow for evaluating extreme subgroup effects under a uniform treatment benefit.",out.width = "100%", out.height = "100%", echo = FALSE}
if (!requireNamespace("DiagrammeR", quietly = TRUE)) {
  message("DiagrammeR not available -- skipping diagram.")
  knitr::knit_exit()
}
...
digraph simulation_flow {
  ...
  # -- Bottom row: simulation loop + results (flows left to right) --

  PartIV [
    label     = "Part IV\\nMain Simulation Loop\\n(n = 5 000 trials)\\n\\nFit stratified Cox\\nin each of 56 subgroups\\nRecord HR and UB(HR)",
    shape     = rectangle,
    style     = filled,
    fillcolor = "#eaf4ea",
    color     = "#1e8449"
  ...
  # -- Force top-row nodes onto the same rank (left-to-right) --
  { rank = same; Source; PartI; PartII; CalibOK; PartIII }

  # -- Force bottom-row nodes onto the same rank (left-to-right) --
  { rank = same; PartIV; Results; Plots }

  # -- Calibration feedback sits between the two rows --
  { rank = same; Calibrate }
  ...
  # -- Drop from top row to bottom row --
  PartIII -> PartIV  [label = "repeat\\nx 5 000"]

  # -- Edges: bottom row --
  PartIV  -> Results
  Results -> Plots   [label = "gg_forest()"]
