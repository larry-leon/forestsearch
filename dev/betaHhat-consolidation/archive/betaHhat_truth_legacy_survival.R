# betaHhat_truth_legacy_survival.R -- LEGACY ARCHIVE, DO NOT USE FOR SCORING
# ===========================================================================
#
# This is the PRE-CONSOLIDATION survival betaHhat module, taken verbatim from
# git history (quarto/simulations/gbsg_redux/betaHhat_truth.R at b0bde64, the
# commit immediately before the shims landed at a87be76).  The only edit is a
# `_legacy` suffix on every definition and every internal call site, so that
# sourcing this file cannot shadow the live shims.
#
#   build_eval_frame_legacy()              .beta_region_legacy()
#   betaHhat_one_legacy()                  betaHhat_table_legacy()
#   attach_betaHhat_legacy()               rule_covs_legacy()
#   betaHhat_theta_dagger_check_legacy()
#
# ---------------------------------------------------------------------------
# IT CARRIES D1 AND D2 BY DESIGN.  THAT IS THE POINT OF KEEPING IT.
# ---------------------------------------------------------------------------
#
# D1 -- the " & " split runs ahead of disjunction dispatch, so a GRF rule such
#   as "(er > 125 & size > 20) | (nodes > 5)" is shredded into fragments naming
#   no real column, and the target comes back NA with two warnings.
# D2 -- partial-NA membership is consumed rather than rejected, so a rule the
#   frame cannot express yields a finite hazard ratio for the WRONG region
#   rather than an all-NA record.
#
# Both are fixed in the package implementation (R/betaHhat_truth.R).  This file
# exists so that the defects remain reproducible: it is the "old" side of the
# T10 migration gate (dev/betaHhat-consolidation/T10_GATE_RESULT.md), and the
# only way to re-derive that comparison once the shims replace the originals.
#
# NEVER source this into a session that scores real targets.  It is not a
# fallback, not a reference implementation, and not a second opinion -- it is a
# preserved bug.  Use forestsearch::fs_betaHhat_one() for anything real.
#
# Replaces: the four byte-identical copies previously at
#   dev/sim-check/{run_pre,fixcheck,run_post,setupcheck}/betaHhat_truth.R
...
#
# NOTE: this file is sourced verbatim by BOTH engines, so build_eval_frame_legacy() no
...
build_eval_frame_legacy <- function(dgm, analysis_time = 84,
                             cens_adjust = log(1.5), eval_seed = 20260628L,
                             n_eval = NULL) {
  if (!is.null(n_eval))
    stop("build_eval_frame_legacy() now evaluates the FULL super-population ",
...
.beta_region_legacy <- function(eval_df, idx, outcome.name, event.name, treat.name) {
...
betaHhat_one_legacy <- function(rule, eval_df,
  ...
  c(betaHhat_H  = .beta_region_legacy(eval_df,  inH, outcome.name, event.name, treat.name),
    betaHhat_Hc = .beta_region_legacy(eval_df, !inH, outcome.name, event.name, treat.name),
...
betaHhat_table_legacy <- function(sg_defs, eval_df,
                           outcome.name = "y_sim", event.name = "event_sim",
                           treat.name = "treat_sim") {
  ...
  do.call(rbind, lapply(u, function(g) {
    v <- betaHhat_one_legacy(g, eval_df, outcome.name, event.name, treat.name)
    data.frame(sg_def      = g,
...
attach_betaHhat_legacy <- function(results, eval_df,
                            outcome.name = "y_sim", event.name = "event_sim",
                            treat.name = "treat_sim") {
  ...
  bt <- betaHhat_table_legacy(results$sg_def, eval_df, outcome.name, event.name, treat.name)
...
betaHhat_theta_dagger_check_legacy <- function(eval_df, harm.name = "flag_harm",
                                        outcome.name = "y_sim",
                                        event.name = "event_sim",
                                        treat.name = "treat_sim") {
  inH <- eval_df[[harm.name]] == 1L
  c(thetaDagger_H  = .beta_region_legacy(eval_df,  inH, outcome.name, event.name, treat.name),
    thetaDagger_Hc = .beta_region_legacy(eval_df, !inH, outcome.name, event.name, treat.name))
}
...
# it splits at strsplit(covs, "+").  rule_covs_legacy() projects that out of the SAME
...
#
#   rec$covs <- paste(rule_covs_legacy(fs.est$sg.harm), collapse = "+")
#
...
rule_covs_legacy <- function(sg.harm) {
