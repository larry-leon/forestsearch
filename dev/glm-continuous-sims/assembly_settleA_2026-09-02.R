...
run_boot40 <- function(label, pargs) {
  cat(sprintf("---- bootstrap 40 reps: %s\n", label))
  set.seed(999L)
  fs_est <- do.call(forestsearch, args_cont())
  t0 <- proc.time()[[3]]
  fb <- suppressWarnings(forestsearch_bootstrap_dofuture(fs_est, nb_boots = 40L, seed = 8316952L,
                                                         details = FALSE,
                                                         parallel_args = pargs))
  wall <- proc.time()[[3]] - t0
  cat(sprintf("  wall %.1f s; per replicate %.2f s\n", wall, wall / 40))
  list(label = label, wall = wall, per_rep = wall / 40, pruned = prune(fb, paste0("boot40_", label)))
}
seq40 <- run_boot40("sequential",   list(plan = "sequential",   workers = 1L,  show_message = FALSE))
par40 <- run_boot40("multisession20", list(plan = "multisession", workers = 20L, show_message = FALSE))
ok <- identical(seq40$pruned, par40$pruned)
cat(sprintf("\npayloads identical (sequential vs multisession/20): %s\n", ok))
if (!ok) { d <- first_diff(seq40$pruned, par40$pruned); cat("  FIRST DIFF at ", d$path, "\n"); print(d$a, digits = 22); print(d$b, digits = 22) }
cat(sprintf("speedup: %.1fx; implied B = 1000 wall at 20 workers: %.1f min (sequential: %.1f min)\n",
            seq40$wall / par40$wall, par40$wall / 40 * 1000 / 60,
...
saveRDS(list(seq = seq40, par = par40, identical = ok,
             ...
             built_at = Sys.time()),
        file.path(D, "assembly_settleA_2026-09-02.rds"))
cat("written:", file.path(D, "assembly_settleA_2026-09-02.rds"), "\n")
