...
forestsearch_bootstrap_dofuture <- function(fs.est,
  ...
  # CRITICAL FIX: Better handling of insufficient bootstrap results
  if (n_found < 2) {
    warning("Only ", n_found, " of ", nb_boots,
            " bootstrap iterations found subgroups. Results unreliable.")

    # Return partial results with NA confidence intervals
    return(list(
      results = results,
      SG_CIs = list(
        H_raw = paste0(round(H_obs, 2), " (NA, NA)"),
        H_bc = "NA (NA, NA)",
        Hc_raw = paste0(round(Hc_obs, 2), " (NA, NA)"),
        Hc_bc = "NA (NA, NA)"
      ),
      FSsg_tab = NULL,
      Ystar_mat = Ystar_mat,
      H_estimates = NULL,
      Hc_estimates = NULL,
      warning = paste("Insufficient bootstrap samples with subgroups:",
                      n_found, "of", nb_boots)
    ))
  }

  # Warn if too few subgroups were found but continue with caution
  if (n_found < nb_boots * 0.5) {
    warning("Only ", round(100 * n_found / nb_boots),
            "% of bootstrap iterations found subgroups. Results may be unreliable.")
  }
  ...
  if (n_found > 0 && !all(is.na(results$H_biasadj_1))) {
    H_estimates <- tryCatch(
      ...
      error = function(e) {
        ...
        warning("Failed to calculate H confidence intervals: ", e$message)
        NULL
      }
    )
  }
  ...
  if (n_found > 0 && !all(is.na(results$Hc_biasadj_1))) {
    Hc_estimates <- tryCatch(
      ...
      error = function(e) {
        ...
        warning("Failed to calculate Hc confidence intervals: ", e$message)
        NULL
      }
    )
  }
