...
#' Supports symmetric trimming to handle extreme values that can distort
#' the display when small subgroups produce very large HR estimates.
#'
...
#' @param trim_fraction Numeric or NULL. Fraction of observations to trim
#'   from each tail (e.g., 0.01 trims the lowest 1\% and highest 1\%).
#'   When non-NULL, trimmed means and SDs are computed for each group,
#'   extreme observations are flagged, and the y-axis is clipped to the
#'   trimmed data range. Set to NULL (default) for no trimming (backward
#'   compatible).
#' @param ylim Numeric vector of length 2 or NULL. Explicit y-axis limits
#'   as \code{c(lower, upper)}. Overrides automatic limits from trimming.
#'   Default: NULL (auto).
#' @param show_summary Logical. Annotate each violin with mean (SD) below
#'   the x-axis labels. When trimming is active, displays trimmed
#'   statistics. Default: TRUE when \code{trim_fraction} is non-NULL,
#'   FALSE otherwise.
#' @param title Character. Plot title. Default: "Distribution of HR
#'   Estimates Across Simulations".
#' @param subtitle Character or NULL. Plot subtitle. When trimming is
#'   active and subtitle is NULL, an auto-generated note indicating the
#'   trim fraction and number of flagged observations is shown.
#'   Default: NULL.
#'
#' @keywords internal
#' @return List with components:
#' \describe{
#'   \item{dfPlot_estimates}{data.table formatted for plotting, with a
#'     \code{trimmed} logical column when trimming is active}
#'   \item{plot_estimates}{ggplot2 object}
#'   \item{trim_info}{List of per-group trimming diagnostics (NULL when
#'     no trimming). Each element contains: \code{n_total}, \code{n_trimmed},
#'     \code{n_flagged}, \code{raw_mean}, \code{raw_sd}, \code{trimmed_mean},
...
SGplot_estimates <- function(
  ...
  # Resolve show_summary default
  if (is.null(show_summary)) {
    show_summary <- !is.null(trim_fraction)
  }
  ...
  hr_estimates <- data.table::rbindlist(
    list(df_itt, df_training, df_testing, df_sg)
  )
  ...
  # ---------------------------------------------------------------------------
  # Trimming (when requested)
  # ---------------------------------------------------------------------------
  trim_info <- NULL
  auto_ylim <- NULL

  if (!is.null(trim_fraction)) {

    if (trim_fraction <= 0 || trim_fraction >= 0.5) {
      stop("trim_fraction must be between 0 (exclusive) and 0.5 (exclusive).")
    }

    hr_estimates[, trimmed := FALSE]
    trim_info <- list()
    total_flagged <- 0L

    for (grp in est_order) {
      idx <- which(hr_estimates$analysis == grp)
      vals <- hr_estimates$est[idx]
      vals_clean <- vals[!is.na(vals)]

      if (length(vals_clean) < 5) {
        trim_info[[grp]] <- list(
          n_total = length(vals), n_trimmed = NA_integer_,
          n_flagged = 0L, raw_mean = mean(vals_clean),
      ...
      # Flag observations outside trim bounds
      is_extreme <- !is.na(hr_estimates$est[idx]) &
        (hr_estimates$est[idx] < lo | hr_estimates$est[idx] > hi)
      hr_estimates[idx[is_extreme], trimmed := TRUE]
    ...
    # Auto y-axis limits from trimmed range (with 5% padding)
    all_bounds <- unlist(lapply(trim_info, function(x) c(x$lower_bound, x$upper_bound)))
    all_bounds <- all_bounds[!is.na(all_bounds)]
    if (length(all_bounds) > 0) {
      rng <- range(all_bounds)
      pad <- 0.05 * diff(rng)
      auto_ylim <- c(rng[1] - pad, rng[2] + pad)
    }

    # Auto subtitle
    if (is.null(subtitle)) {
      subtitle <- sprintf(
        "Trimmed mean/SD shown (%.0f%% symmetric trim; %d observations flagged)",
  ...
  # ---------------------------------------------------------------------------
  # Build x-axis labels with summary annotations
  # ---------------------------------------------------------------------------
  if (show_summary) {
    annotated_labels <- vapply(est_order, function(grp) {
      vals <- hr_estimates$est[hr_estimates$analysis == grp]
      vals <- vals[!is.na(vals)]

      if (!is.null(trim_info) && !is.null(trim_info[[grp]])) {
        ti <- trim_info[[grp]]
        if (!is.na(ti$trimmed_mean)) {
          return(sprintf(
            "%s\nMean*: %.3f (%.3f)",
            grp, ti$trimmed_mean, ti$trimmed_sd
          ))
        }
      }

      # Raw mean/SD (no trimming or insufficient data for trimming)
      if (length(vals) > 1) {
        return(sprintf(
          "%s\nMean: %.3f (%.3f)",
          grp, mean(vals), stats::sd(vals)
        ))
      }
      grp
    }, character(1))

    names(annotated_labels) <- est_order
  }

  # ---------------------------------------------------------------------------
  # Plotting data: use only non-flagged for violins when trimming
  # ---------------------------------------------------------------------------
  if (!is.null(trim_fraction)) {
    plot_data <- hr_estimates[trimmed == FALSE]
  } else {
    plot_data <- hr_estimates
  }
  ...
  p <- ggplot2::ggplot(
    plot_data,
    ggplot2::aes(x = analysis, y = est, fill = analysis)
  ) +
    ggplot2::geom_violin(trim = FALSE, alpha = 0.7) +
    ggplot2::geom_boxplot(width = 0.15, fill = "white", alpha = 0.8) +
    ggplot2::geom_hline(
      yintercept = 1.0, linetype = "dashed", color = "red", alpha = 0.7
    ) +
  ...
  if (show_summary) {
    p <- p + ggplot2::scale_x_discrete(labels = annotated_labels)
  }

  # Y-axis limits: explicit > auto-trim > none
  effective_ylim <- if (!is.null(ylim)) ylim else auto_ylim
  if (!is.null(effective_ylim)) {
    p <- p + ggplot2::coord_cartesian(ylim = effective_ylim)
  }

  return(list(
    dfPlot_estimates = hr_estimates,
    plot_estimates = p,
    trim_info = trim_info
  ))
}
