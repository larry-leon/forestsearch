# Comparison A driver — payload-to-payload, field by field, full precision
# (task: cc_task_applications_render_0.3.5_2026-09-01.md §4)
#
# Pairs each _payloads_2026-09-01/ file with its _payloads_2026-08-31/ counterpart by name
...
#
# Reference for comparison_continuous.rds: the in-repo _payloads_2026-08-31/ set excludes
# it (>50 MB, commit 8ad2c3f6); the 08-31 original survives read-only in
# ~/fs_parked_2026-08-31/ and is used as the reference for that one file (deviation
# documented in the report).

OUT <- Sys.getenv("FS_COMPARE_OUT", "/tmp/fs_compare_A_2026-09-01.md")
REPO <- "."
...
reset_rec <- function() {
  ...
  REC$noncomp <- list(); REC$normeq <- list()
}
...
cmp <- function(a, b, path, depth = 0L) {
  if (is_volatile_path(path)) {
    rec("volatile", path, paste0("A(08-31): ", fmt(a, 300), "\nB(09-01): ", fmt(b, 300)))
    return(invisible())
  }
  ...
  if (is.list(a) && is.list(b) && !is.function(a)) {
    ...
    if (!is.null(na) && !is.null(nb) && !anyDuplicated(na) && !anyDuplicated(nb) &&
      ...
      for (n in setdiff(na, nb)) rec("structural", paste0(path, "/", n), "present in A(08-31) only")
      for (n in setdiff(nb, na)) rec("structural", paste0(path, "/", n), "present in B(09-01) only")
...
enum_volatile <- function(a, b, path, acc, depth = 0L) {
  if (is_volatile_path(path)) {
    acc[[length(acc) + 1L]] <- c(path = path, txt = paste0("A(08-31): ", fmt(a, 300),
                                                          "\nB(09-01): ", fmt(b, 300)))
    return(acc)
  }
  ...
  if (is.list(b) && !is.function(b) && !is.null(names(b)) && all(nzchar(names(b)))) {
    for (n in names(b)) {
      av <- if (is.list(a) && n %in% names(a)) a[[n]] else "<absent in A>"
...
pairs <- list(
  list(name = "gbsg multimethod payload",
       ref = "quarto/applications/gbsg/_payloads_2026-08-31/analysis_gbsg_survival_multimethod/analysis_gbsg_survival_multimethod_payload.rds",
       new = "quarto/applications/gbsg/_payloads_2026-09-01/analysis_gbsg_survival_multimethod/analysis_gbsg_survival_multimethod_payload.rds"),
  list(name = "gbsg frozen_family payload",
       ref = "quarto/applications/gbsg/_payloads_2026-08-31/analysis_gbsg_survival_frozen_family/analysis_gbsg_survival_frozen_family_payload.rds",
       new = "quarto/applications/gbsg/_payloads_2026-09-01/analysis_gbsg_survival_frozen_family/analysis_gbsg_survival_frozen_family_payload.rds"),
  list(name = "actg175 template payload",
       ref = "quarto/applications/actg175/_payloads_2026-08-31/template_actg175_continuous/template_actg175_continuous_payload.rds",
       new = "quarto/applications/actg175/_payloads_2026-09-01/template_actg175_continuous/template_actg175_continuous_payload.rds"),
  list(name = "compare_all selected_subgroups_continuous",
       ref = "quarto/applications/actg175/_payloads_2026-08-31/analysis_actg175_continuous_compare_all/selected_subgroups_continuous.rds",
       new = "quarto/applications/actg175/_payloads_2026-09-01/analysis_actg175_continuous_compare_all/selected_subgroups_continuous.rds"),
  list(name = "compare_all comparison_continuous (ref = parked 08-31 original)",
       ref = "/home/larryleon/fs_parked_2026-08-31/_payloads_2026-08-31/analysis_actg175_continuous_compare_all/comparison_continuous.rds",
       new = "quarto/applications/actg175/_payloads_2026-09-01/analysis_actg175_continuous_compare_all/comparison_continuous.rds")
)

sink(OUT)
cat("# Comparison A — payload-to-payload, 0.3.5 (2026-09-01) vs 0.3.1 (2026-08-31)\n\n")
overall_clean <- TRUE
for (p in pairs) {
  cat("\n## ", p$name, "\n\nref: `", p$ref, "`\nnew: `", p$new, "`\n\n", sep = "")
...
cat("\n\n# Named selection lines (task §4)\n\n")
nm <- function(path, get) { x <- readRDS(path); get(x) }
for (side in c("2026-08-31", "2026-09-01")) {
  ...
  tp  <- sprintf("quarto/applications/actg175/_payloads_%s/template_actg175_continuous/template_actg175_continuous_payload.rds", side)
  ss  <- sprintf("quarto/applications/actg175/_payloads_%s/analysis_actg175_continuous_compare_all/selected_subgroups_continuous.rds", side)
  cat("## ", side, "\n", sep = "")
  ...
  cat("- actg175 template sg_harm: ", deparse(nm(tp, function(x) x$labels$sg_harm)), "\n", sep = "")
  s <- readRDS(ss)
  cat("- actg175 compare_all subgroups:\n")
  for (i in seq_len(nrow(s)))
    cat("    ", s$combo_label[i], " => ", s$subgroup[i], "  (N_H=", s$N_H[i],
        ", effect=", sprintf("%.17g", s$effect[i]), ", Pcons=",
        sprintf("%.17g", s$Pcons[i]), ")\n", sep = "")
}
