...
#'   (upper-bound) estimate only.  Ignored when \code{n_eff} is set.
#' @param n_eff Numeric or \code{NULL}.  Pre-specified effective number
#'   of independent tests.  When set, overrides the \code{rho}-based
#'   Sidak adjustment.  Default \code{NULL} (compute from \code{rho}).
...
fpr_approx <- function(
  ...
  if (effect_type == "ratio") {
    # Log-OR SE from expected 2x2 cell counts
  ...
  # Correlation-adjusted (Sidak-like with effective number of tests)
  if (!is.null(n_eff)) {
    # User-specified effective number of tests
    fpr_adjusted <- 1 - (1 - per_candidate)^n_eff
  } else if (rho > 0 && n_candidates > 1) {
...
print.fpr_approximation <- function(x, ...) {
  ...
  cat(sprintf("  Confounders:        %d continuous (x%d cuts) + %d binary\n",
  ...
  if (p$n_eff_user) {
    cat(sprintf("  Effective tests:    %.0f  (user-specified)\n", x$n_eff))
  } else if (p$rho > 0) {
