...
#'   * `sg.harm.id` Character vector defining the identified subgroup
#'     (e.g., `c("v1=1", "v3=1")`), or `NULL` if no subgroup was found.
#'   * `data` The input data frame with a `treat.recommend` column added
#'     (`0` = in harm/questionable subgroup, `1` = complement).
#'   * `tree.cuts` Named list of GRF policy-tree split points.
#'   * `grf_varimp` Named numeric vector of GRF variable importances.
#'   * `effect_measure` Character: the effect measure used.
#'   * `outcome_type` Character: the outcome type.
#'   * `overdispersion` Character: the overdispersion correction applied.
#'   * `offset.name` Character or `NULL`: the offset variable name.
#'   * `grf_count_transform` Character: the transform argument as matched
#'     (`"log"` or `"identity"`).
#'   * `grf_y_transform` Character: transformation actually applied to Y
#'     for GRF screening (`"log(Y + 0.5)"`, `"identity"`, or `"none"`).
#'
#' @details
...
#'   \code{\link{glm_cs_fit}} for continuous biomarker profiles.
#'
