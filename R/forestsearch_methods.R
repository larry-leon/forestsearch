...
summary.forestsearch <- function(object, ...) {
  ...
  if (!is.null(params)) {
    ...
    # Candidate-family status: "fixed" (a fixed family, the condition
    # multiplier resampling requires), "conditional-removable" (a front end is
    # on; turning it off makes the family fixed), or "conditional-inherent"
    # (DINA/GRF, where the family is generated by fitting a model).
