...
- hr_threshold:
  ...
  Numeric. HR threshold for detection (e.g., 1.25). This is the
  threshold that the average HR across splits must exceed.

- hr_consistency:

  Numeric. HR consistency threshold (e.g., 1.0). This is the threshold
  each individual split must exceed. Default: 1.0
...
If theta is scalar, returns a single probability. If theta is a vector,
returns a data.frame with columns: theta, probability.

## Details

This function computes P(detect \| theta) using the asymptotic normal
approximation for the log hazard ratio estimator. The detection
criterion is based on ForestSearch's split-sample consistency
evaluation:
